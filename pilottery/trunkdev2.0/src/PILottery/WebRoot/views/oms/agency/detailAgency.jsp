<%@ page language="java" contentType="text/html; charset=utf-8" pageEncoding="utf-8"%>
<%@ include file="/views/common/taglibs.jsp"%>
<html>
<head>
<title>销售站详情</title>

<link rel="stylesheet" type="text/css" href="${basePath}/css/style.css" />
<link rel="stylesheet" type="text/css" href="${basePath}/css/pop-up.css" />
<link rel="stylesheet" type="text/css" href="${basePath}/css/validate.css" />
<script type="text/javascript" src="${basePath}/js/oms/myJs.js"></script>
<script type="text/javascript" src="${basePath}/js/jquery/jquery-1.9.1.js"></script>

<script type="text/javascript" charset="UTF-8" > 

</script>
</head>
<body style="margin:0;">
<div id="tck" >
		<div class="mid">
			<table width="100%" border="0" cellspacing="0" cellpadding="0" >
				<tr>
				    <td align="right" class="td" width="25%">Outlet Code：</td>
				    <td  align="left" class="td" width="20%">${agency.agencyCode}	</td>		
				    <td  class="td" align="right" width="25%">Outlet Name：</td>
        			<td class="td" align="left" > ${agency.agencyName}</td>
      			</tr>
      			
      			<tr>
			       <td class="td"  align="right">Contact：</td>
			        <td class="td" align="left">${agency.agencyManager }</td>  
			        <td  class="td" align="right">Phone：</td>
			        <td class="td" align="left">${agency.agencyPhone}</td> 
     			 </tr>
      			
      			<tr>
			       <td class="td"  align="right">Bank Name：</td>
			        <td class="td" align="left">${agency.agencyBank.bankName}</td>  
			        <td  class="td" align="right">Bank Account：</td>
			        <td class="td" align="left">${agency.bankAccount}</td> 
     			 </tr>
      			
      			<tr>
			       <td class="td"  align="right">Contract No：</td>
			        <td class="td" align="left">${agency.contractNo}</td>  
			        <td  class="td" align="right">Outlet Address：</td>
			        <td class="td" align="left">${agency.agencyAddress}</td> 
     			 </tr>
			</table>
			</div>
			
		 <div style="padding:0 20px 20px 20px;">
       <div style="position:relative; z-index:1000px;margin-top: 10px;">
         <table class="datatable" id="table1_head" width="100%">
           <tbody><tr>
             <th width="20%">Terminal Code</th>
             <th width="2%">|</th>
             <th width="22%">MAC Address</th>
             <th width="2%">|</th>
             <th width="22%">Status</th>
             <th width="2%"></th>
           </tr>
         </tbody></table>
       </div>	
		
		<div id="box" style="border:1px solid #ccc;">
         <table id="fatable" class="datatable" cellpadding="0" cellspacing="0" width="100%">
           <tbody>
            <c:forEach var="data" items="${agencyTerminal }">
           	<tr>
              <td width="20%">
             	${data.terminalCode }
              </td>
              <td width="2%">&nbsp;</td>
              <td width="22%">${data.macAddress }</td>
              <td width="2%">&nbsp;</td>
              <td width="22%">
              	<c:if test="${data.status == 1}">Enabled</c:if>
              	<c:if test="${data.status == 2}">Disabled</c:if>
              	<c:if test="${data.status == 3}">Removed</c:if>
              </td>
              <td width="2%">&nbsp;</td>
            </tr>
            </c:forEach>
         </tbody></table>
       </div>
		</div>

<!-- 销售员 -->
	<div style="padding:0 20px 20px 20px;">
       <div style="position:relative; z-index:1000px;margin-top: 10px;">
         <table class="datatable" id="table1_head" width="100%">
           <tbody><tr>
             <th width="20%">Teller Code</th>
             <th width="2%">|</th>
             <th width="22%">Teller Name</th>
             <th width="2%">|</th>
             <th width="22%">Teller Type</th>
             <th width="2%">|</th>
             <th width="22%">Teller Status</th>
             <th width="2%"></th>
           </tr>
         </tbody></table>
       </div>	
		
		<div id="box" style="border:1px solid #ccc;">
         <table id="fatable" class="datatable" cellpadding="0" cellspacing="0" width="100%">
           <tbody>
            <c:forEach var="data" items="${agencyTeller }">
           	<tr>
              <td width="20%">
             	${data.tellerCode }
              </td>
              <td width="2%">&nbsp;</td>
              <td width="22%">${data.tellerName }</td>
              <td width="2%">&nbsp;</td>
              <td width="22%">
              	<c:if test="${data.newTellerType == 1}">Common Teller</c:if>
              	<c:if test="${data.newTellerType == 2}">Outlet Manager</c:if>
              	<c:if test="${data.newTellerType == 3}">Trainee</c:if>
              </td>
              <td width="2%">&nbsp;</td>
              <td width="22%">
              	<c:if test="${data.newTellerStatus == 1}">Enabled</c:if>
              	<c:if test="${data.newTellerStatus == 2}">Disable</c:if>
              	<c:if test="${data.newTellerStatus == 3}">Deleted</c:if>
              </td>
              <td width="2%">&nbsp;</td>
            </tr>
            </c:forEach>
         </tbody></table>
       </div>
	</div>
    </div>
</body>
</html>
