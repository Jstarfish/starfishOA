...
public class RoleController {
	...
	public String toRoleList(HttpServletRequest request, ModelMap model,
		...
		return LocaleUtil.getUserLocalePath("system/roleList", request);
	}
	...
	public String editRole(HttpServletRequest request, ModelMap model,
		...
		return LocaleUtil.getUserLocalePath("system/editRole", request);
	}
	...
	public String addRoleUser(HttpServletRequest request, ModelMap model,
		...
		return LocaleUtil.getUserLocalePath("system/addRoleUsers", request);
	}
	...
	public String saveRoleUser(HttpServletRequest request, ModelMap model) {
		try {
			...
			if (roleid>0) {
				...
				if(r.getC_errcode() != 0)
				{
					throw new Exception(LocaleUtil.getUserLocaleErrorMsg(r.getC_errmsg(), request));
				}
			}else
			{
	...
	public String rolePrivilege(HttpServletRequest request, ModelMap model) {
		...
		//汉化补丁
		String jsonStr = roleService.getTree(list, user.getUserLang());
		model.addAttribute("jsonStr", jsonStr);

		return LocaleUtil.getUserLocalePath("system/rolePermissions", request);
	}
	/**
	 *  修改角色授权信息
	 */
