...
public abstract class HttpClientUtils {
	...
	public static String postString(String urlStr,String params){
		...
		try {
			...
			log.error("HTTP connect timeout!");
			result="connectTimeOut";
			e.printStackTrace();
		}catch (SocketTimeoutException e) {
			result="connectTimeOut";
			e.printStackTrace();
			log.error("send post request timeout!");
		}
		catch (Exception e) {
			e.printStackTrace();
			log.error("send post request error!" + e);
	 ...
	 * 2、2xx消息之外的，也算错误处理
	 */
	public static String postStringForBank(String urlStr,String params){
		HttpURLConnection con = null;
		String result = null;
		PrintWriter printWriter = null;
		BufferedReader bufferedReader = null;
		...
		try {
			...
			if (responseCode < 200 || responseCode>299) {
				log.error(" Error===" + responseCode);
				throw new Exception("Net status is error,error code is:"+responseCode);
			}
