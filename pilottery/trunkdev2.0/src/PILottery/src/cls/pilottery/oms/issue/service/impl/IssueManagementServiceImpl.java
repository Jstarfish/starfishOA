...
public class IssueManagementServiceImpl implements IssueManagementService {
	...
	public void updateGameIssuePrizeRule(List<GameIssuePrizeRule> list) {
		for(Iterator<GameIssuePrizeRule> it = list.iterator();it.hasNext();){
			GameIssuePrizeRule rule = it.next();
			issueManagementDao.updateGameIssuePrizeRule(rule);
		}
	}
	...
	public String insertIssuesP(List<GameIssue> issueList) {
		...
		try {
			conn = jdbcTemplate.getDataSource().getConnection();// 通过注解的方式获取数据源，并获取连接
