...
public class IssueManagementController {
	...
	public String issueParam(HttpServletRequest request, ModelMap model, Short gameCode, Long issueNumber) {
		...
		// 游戏风控参数
		GameParameterHistory riskControlParameter = gameManagementService.getGameParameterHistoryByNum(gameInfo.getGameCode(),issueNumber);
		...
		if ((gameInfo.getGameCode() == SysConstants.KOC11X5 || gameInfo.getGameCode() == SysConstants.TEMA )&& riskControlParameter != null) { //11选5
			try {
			...
			try {
				...
				if (riskParam != null && !riskParam.equals("")) {
					//时时彩中取得参数为0.5;1:50000,2:5000,4:5000,5:500,8:781,9:1562,10:50,12:50,13:100000 分号前数字是返奖率
					String[] risk = riskParam.split(";");
					riskParam = risk[1];
					//riskParam = riskParam.substring(0, riskParam.indexOf(";"));
	...
	public String pre_qlx_tty(HttpServletRequest request, ModelMap model, Short gameCode) {
		...
		if (gameCode == SysConstants.KOC7LX || gameCode == SysConstants.C30S6 || gameCode == SysConstants.TEMA) {
			...
			if (maxIssueNumber == null) {
				...
				String year = sdf.format(new Date());
				maxIssueNumber = new Long(year + "00000");
			}
			form.setBeginIssue(maxIssueNumber + 1);
		}
		model.addAttribute("games", games);
	...
	public String preview_qlx_tty(HttpServletRequest request, HttpServletResponse response, ModelMap model, IssueManagementForm form) throws Exception {
		...
		} else if (gameCode == SysConstants.TEMA) {
			planStartHour = param.get("planStartHour_tema");
			planCloseHour = param.get("planCloseHour_tema");
			drawDays = RegexUtil.getString2intArr(param.get("drawDays_tema"));
		}
		...
		if (tINo == null) {
			...
			if (lastIssue.getPlanCloseTime().getTime() > realbeginTime.getTime()) {
				if(LocaleUtil.isChinese(request)){
					model.addAttribute("reservedSuccessMsg", "开始时间必须晚于最后一期的截止时间");
				}else{
					...
					if (issueNo == 0) {
						...
						if (lastStartDate != null) {
							...
							if (!year_lastStartTime.equals(year_currentStartTime))
								beginIssue = new Long(year_currentStartTime + "00001");
						}
	...
	private String previewIssue(HttpServletRequest request, ModelMap model, IssueManagementForm form) throws ParseException {
		...
		for (int i = 1; i <= drawTimesEachDay; i++) {
			...
			Date issueCloseDate = null;

			if (planStartHour.indexOf("last") > -1) {
				planStartHour = planStartHour.split(",")[1];
				issueStartDate = sdf2.parse(sdf3.format(sdf1.parse(issueDate)) + " " + " " + planStartHour);
				issueStartDate = new Date(issueStartDate.getTime() - (long) 24 * 60 * 60 * 1000);
			} else {
				issueStartDate = sdf2.parse(sdf3.format(sdf1.parse(issueDate)) + " " + " " + planStartHour);
			}
			if (planCloseHour.indexOf("next") > -1) {
				planCloseHour = planCloseHour.split(",")[1];
				issueCloseDate = sdf2.parse(sdf3.format(sdf1.parse(issueDate)) + " " + planCloseHour);
				issueCloseDate = new Date(issueCloseDate.getTime() + (long) 24 * 60 * 60 * 1000);
			} else {
				issueCloseDate = sdf2.parse(sdf3.format(sdf1.parse(issueDate)) + " " + planCloseHour);
			}
			...
			gi.setPlanCloseTime(issueCloseDate);
			gi.setPlanRewardTime(issueCloseDate);
			issueList.add(gi);
			issueNo++;
