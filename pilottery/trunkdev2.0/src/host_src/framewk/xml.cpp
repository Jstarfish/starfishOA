...
{
public:
    Base64Encoder(){its3Len=0; itsLinePos=0;memset(itsCurr3,0,3);}
...
{
public:
    Base64Decoder(){its4Len=0;itsEnded=0;itsErrNum=0;memset(itsCurr4,0,4);}
    virtual OutIter Filter( OutIter out, InIter inBeg, InIter inEnd );
    virtual OutIter Finish( OutIter out );
private:
    int             itsEnded;
...
public:
    ...
    void* Decrypt(const char * srcp, int len, char * dstp)
    {
        ...
        while(i < len)
        {
            *dstp++ = (B64_offset[(int)*(srcp+i)] <<2 | 
                                B64_offset[(int)*(srcp+i+1)] >>4);
            *dstp++ = (B64_offset[(int)*(srcp+i+1)]<<4 | 
                                B64_offset[(int)*(srcp+i+2)]>>2);
            *dstp++ = (B64_offset[(int)*(srcp+i+2)]<<6 |
                                B64_offset[(int)*(srcp+i+3)] );
            i += 4;
        }
        srcp += i;
        ...
        if (root->xfformat.ElementsNoBreak == false || TotalContents != 1 || TotalComments || TotalCDatas || iC)
            ...
            if (TargetMode == 2)
                ...
                if (TargetMode == 3)
                    ...
                    //caoxf__ fprintf(fp,">",b.operator char*());
                    fprintf(fp,">");
            }
        }
