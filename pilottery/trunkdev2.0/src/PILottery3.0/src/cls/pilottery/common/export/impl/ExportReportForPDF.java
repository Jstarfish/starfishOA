...
public class ExportReportForPDF implements IExportReport{
	...
	public void createReport(HttpServletRequest request,
		...
		try {
			...
			//Locale local = SpringContextUtil.getLocal(request);
			User user = (User) request.getSession().getAttribute(SysConstants.CURR_LOGIN_USER_SESSION);
			boolean isEn = user.getUserLang().equals(UserLanguage.EN);
			...
			if(isEn){
				 ...
				 font=ReportFontFactory.getFontChinese(Font_Type.CONTENT);
				 titleFont = ReportFontFactory.getFontChinese(Font_Type.CONTENT);
				 titleFont.setSize(20);
				 titleFont2 = ReportFontFactory.getFontChinese(Font_Type.CONTENT);
				 titleFont2.setSize(10);
				 footer = new HeaderFooter(new Phrase("第 ",ReportFontFactory.getFontChinese(Font_Type.CONTENT)),new Phrase(" 页",ReportFontFactory.getFontChinese(Font_Type.CONTENT)));
			...
			if(StringUtils.isNotEmpty(reportTitle) && !reportTitle.equals("undefined")){
									...
									for (int l = 0; l < datapdf.length; l++) {
											...
											if(isEn){
												if(value.trim().matches(reg)){
												  paragraph=new Paragraph(value , ReportFontFactory.getFontChinese(Font_Type.CONTENT));
												}
												else{
													paragraph=new Paragraph(value , ReportFontFactory.getFontEnglish(Font_Type.CONTENT));
												}
											}
											else{
												paragraph=new Paragraph(value , ReportFontFactory.getFontChinese(Font_Type.CONTENT));
