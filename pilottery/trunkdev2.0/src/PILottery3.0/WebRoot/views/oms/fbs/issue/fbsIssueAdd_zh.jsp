...
<head>
    ...
    <script type="text/javascript" charset="UTF-8">
        ...
        function checkFormData() {
            ...
            if (issue == "") {
                document.getElementById("codeTip").innerHTML = "请输入期次编号";
                $("#codeTip").css("color", "red");
                return false;
            } else {
                if (oldIssue > issue || oldIssue.length > issue.length) {
                    document.getElementById("codeTip").innerHTML = "请输入合法期次编号";
                    $("#codeTip").css("color", "red");
                    return false;
                }
            }
            if (start == "") {
                document.getElementById("startTip").innerHTML = "请输入期次开始时间";
                $("#startTip").css("color", "red");
                return false;
            }
            if (end == "") {
                document.getElementById("endTip").innerHTML = "请输入期次结束时间";
                $("#endTip").css("color", "red");
                return false;
            }
            if (datev == "") {
                document.getElementById("dateTip").innerHTML = "请输入期次日期";
...
<form:form modelAttribute="issueAddForm" id="issueAddForm" method="post">
    <div class="pop-body">
        <table width="100%" border="0" cellspacing="0" cellpadding="0" style="height: 120px;">
            <tr>
                <td align="right" width="30%">期次编号：</td>
                <td>
                    <input id="oldIssueCode" name="oldIssueCode" value="${maxIssue}"
                           type="hidden"/>
                    <input id="fbsIssueCode" name="fbsIssueCode" value="" class="text-normal"
                           type="text" placeholder="最大期次:${maxIssue}"/>
            ...
            <tr>
                <td align="right" width="30%">期次日期：</td>
                <td>
