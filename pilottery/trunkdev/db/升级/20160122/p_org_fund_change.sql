...
begin
   -- 按照类型，处理正负号
   case
      when p_type in (eflow_type.charge, eflow_type.org_comm, eflow_type.org_return, eflow_type.org_comm_agency_sale, eflow_type.org_comm_pay, eflow_type.paid) then
         v_amount := p_amount;
         v_frozen := 0;

      when p_type in (eflow_type.withdraw, eflow_type.carry, eflow_type.org_comm_agency_return, eflow_type.org_comm_agency_sale) then
         v_amount := 0 - p_amount;
         v_frozen := 0;
