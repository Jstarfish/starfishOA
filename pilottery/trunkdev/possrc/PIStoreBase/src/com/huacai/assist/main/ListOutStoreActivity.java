 ...
 *
 */
public class ListOutStoreActivity extends BaseActivity implements RequestListener, OnItemClickListener, OnSetItemListener{
	private ListView lv;
	...
	public void initView() {
		...
		super.initView();
		setMiddleText(R.string.string_out_store_list);//设置标题
	...
	public void sendInitialMsg(){
		...
		Http.request(Http.STORE_OUT_LIST, jo, this);//发送查询数据的请求
	}
	...
	public void onComplete(NetResult res) {//接收网络数据
		...
		// 测试信息
		String fackJson = App.getResRawString(R.raw.r070008);
		try {
			res.errorCode = 0;
				...
				if (res.errorCode == 0 && errCode == 0) {//如果错误码位0
					...
					this.runOnUiThread(new Runnable() {
						...
						public void run() {//更新出货单列表
							adapter = new YYcAdapter(ListOutStoreActivity.this,
									R.layout.list_four_item,
									resultData, "recordList",
									new String[]{"time", "receiveUnit", "type", "amount", "typeValue", "status", "sgrNo"},
									new String[]{"time",		"", 		"",		"longR", 	"",			"",		""});
							adapter.setOnSetItemListener(ListOutStoreActivity.this);
...
//			data[6]; // dealNo
			if (data[4].equals("1")) {
				// 调拨单入库
				startActivity(new Intent(this, SelectOutMoveStoreListActivity.class));
			} else if (data[4].equals("2")) {
				// 出货单出库
				startActivity(new Intent(this, SelectOutStoreListActivity.class));
			} 
		}
	}
	@Override
