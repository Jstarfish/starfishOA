...
function deleteWarehousePrompt(warehouseCode) {
	var msg = "Are you sure you want to delete warehouse <b>" + warehouseCode + "</b>?";
	showDialog("deleteWarehouseAjax('"+warehouseCode+"')", "Delete Warehouse", msg);
}
...
<body>
    <!-- 顶部标题块 -->
    <div id="title">Warehouses</div>
    
    <!-- 查询条件块 -->
    <div class="queryDiv">
        <form action="warehouses.do?method=listWarehouses" method="POST" id="warehouseQueryForm">
            <div class="left">
                <span>Warehouse Code: <input id="warehouseCodeQuery" name="warehouseCodeQuery" value="${warehouseQueryForm.warehouseCodeQuery}" class="text-normal" maxlength="40"/></span>
                <span>Institution: 
            ...
            <div class="right">
                <table width="260" border="0" cellspacing="0" cellpadding="0">
                    <tr style="height:50px;line-height:50px">
                        <td align="right">
                            <input type="button" value="New Warehouse" onclick="showBox('warehouses.do?method=addWarehouseInit','New Warehouse', 650, 800);" class="button-normal"></input>
                        </td>
                    </tr>
                </table>
            </div>
                ...
                <td>
                    <table class="datatable">
                        <tr class="headRow">
                            ...
                            <td style="width:14%">Institution</td>
                            <td width="1%">|</td>
                            <td style="width:10%">Warehouse Code</td>
                            <td width="1%">|</td>
                            <td style="width:14%">Warehouse Name</td>
                            <td width="1%">|</td>
                            <td style="width:20%">Warehouse Address</td>
                            <td width="1%">|</td>
                            <td style="width:10%">Director</td>
                            <td width="1%">|</td>
                            <td style="width:10%">Warehouse Status</td>
                            <td width="1%">|</td>
            ...
            <c:forEach var="row" items="${pageDataList}" varStatus="status">
                <tr class="dataRow">
                    ...
                    <c:if test="${row.status == 1}">
                    	<td style="width:10%" title="Operational">Operational</td>
                    </c:if>
                    <c:if test="${row.status == 2}">
                    	<td style="width:10%" title="Deleted">Deleted</td>
                    </c:if>
                    <c:if test="${row.status == 3}">
                    	<td style="width:10%" title="Checking">Checking</td>
                    </c:if>
                    <td width="1%">&nbsp;</td>
                    <td style="width:*%">
                        <span><a href="#" onclick="showBox('warehouses.do?method=warehouseDetails&curWarehouseCode=${row.warehouseCode}','Warehouse Details', 650, 800);">Details</a></span>&nbsp;|
                        <span><a href="#" onclick="showBox('warehouses.do?method=modifyWarehouseInit&curWarehouseCode=${row.warehouseCode}','Edit Warehouse', 650, 800);">Edit</a></span>&nbsp;|
                        <span><a href="#" onclick="deleteWarehousePrompt('${row.warehouseCode}');">Delete</a></span>
                    </td>
                </tr>
            </c:forEach>
        </table>
