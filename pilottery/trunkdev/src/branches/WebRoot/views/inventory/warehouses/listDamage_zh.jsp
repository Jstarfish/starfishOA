			...
			<div class="left">
				<span>方案名称: <select name="planCodeQuery" id="planCodeQuery"
					class="select-normal" onchange="initSelect($('#planCodeQuery').val());"><option
						...
						</c:forEach></select></span> <span>批次编号: <select name="batchNoQuery" id="batchNoQuery"
					class="select-normal"></select></span> <input type="button" value="查询"
			...
			<c:forEach var="items" items="${pageDataList }" varStatus="status">
				<tr class="dataRow">
					...
					<td style="width: *%"><span><a href="#"
							onclick="showBox('damagedGoods.do?method=detailsDamaged&record=${items.recordCode }','损毁详情',650,980)">详情</a></span>
					</td>
				</tr>
			</c:forEach>
		</table>
