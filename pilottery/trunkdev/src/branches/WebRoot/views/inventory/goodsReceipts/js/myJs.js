...
function deleteGame1(rowNum,obj){
	 ...
	 SJZPrinter.removeObject(del);
        var unitype=$("#packUnit"+rowNum).val();
			        var planCode=$("#planCode"+rowNum).val();
					if(unitype=='Trunk'){
						var trunkamout=parseInt(formatNum($("#plantrunkAmount"+planCode).html()))-1;
						$("#plantrunkAmount"+planCode).html(formatNumber(trunkamout));
					}
					if(unitype=='Box'){
						var boxAmount=parseInt(formatNum($("#plantrunkBox"+planCode).html()))-1;
						$("#plantrunkBox"+planCode).html(formatNumber(boxAmount));
					}
					if(unitype=='Package'){
						var pkgAmount=parseInt(formatNum($("#plantrunkPkg"+planCode).html()))-1;
						$("#plantrunkPkg"+planCode).html(formatNumber(pkgAmount));
					}
				
				
					//var titickts=parseInt($("#plantrunkTickts"+obj.planCode).html())-$("#ticketNum"+rowNum).val();
			
					 var num =formatNum($("#plantrunkTickts"+planCode).html());
					
					var titickts=parseInt(num)-$("#ticketNum"+rowNum).val();
					$("#plantrunkTickts"+planCode).html(formatNumber(titickts));
					var sum=formatNum($("#sumTickets").html());
					$("#sumTickets").html('');
					var titcks=$("#ticketNum"+rowNum).val();
					var total=formatNum(sum)-titcks;
					$("#sumTickets").html(formatNumber(total));
				$(obj).parent().parent().remove(); 
