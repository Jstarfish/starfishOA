...
function newReturnDelivery(){
	showBox('returnDelivery.do?method=initReturnDeliveryAdd','新建还货',650,800);
}

function showDetails(stbNo){
	showBox('returnDelivery.do?method=returnDeliveryDetail&queryNo=' + stbNo,'还货详情',650,800);
}

function editOrder(stbNo){
	showBox('returnDelivery.do?method=initReturnDeliveryEdit&queryNo=' + stbNo,'编辑还货',650,800);
}

function approve(stbNo){
	showBox('returnDelivery.do?method=initAproveReturnDelivery&queryNo=' + stbNo,'还货申请',650,800);
}
...
function cancelOrder(stbNo,status) {
	...
	showDialog("doCancel('"+url+"')",'取消','你确定要取消<br/> 还货 : '+stbNo);
}
...
<body>
    <div id="title">还货管理</div>
    
    <div class="queryDiv">
        <form action="returnDelivery.do?method=listReturnDeliveries" id="queryForm" method="post">
            <div class="left">
                <span>还货编号: <input id="queryNo" name="queryNo" value="${form.queryNo}" maxlength="10" class="text-normal"/></span>
                <span>还货日期:
