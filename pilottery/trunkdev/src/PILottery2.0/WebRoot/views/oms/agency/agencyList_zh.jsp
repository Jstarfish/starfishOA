            ...
            <table class="datatable" id="exportPdf">
                <tr class="headRow">
                             <td style="width: 10px;">&nbsp;</td>
                       		 <td style="width:7%" title="销售站编码">销售站编码</td>
                       		 <th width="1%">|</th>
							<td style="width:10%" title="销售站名称">销售站名称</td>
							 <th width="1%">|</th>
							<td style="width:10%" title="区域名称">区域名称</td>
							 <th width="1%">|</th>
							<td style="width:5%" title="销售站状态">销售站状态</td>
							 <th width="1%">|</th>
							<td style="width:5%">销售站类型</td>
							 <th width="1%">|</th>
							<td style="width:10%" title="永久信用额度">永久信用额度</td>
							 <th width="1%">|</th>
							<td style="width:10%" title="临时信用额度">临时信用额度</td>
							 <th width="1%">|</th>
							<td style="width:8%" title="账户余额">账户余额</td>
							 <th width="1%">|</th>
			...
			<tr class="dataRow">
				...
				<td style="width:*%" class="no-print">
                 	<c:if test="${data.agencyStatus.value == 1 && data.agencyType.value != 4}">
						...
						<span><a href="#" onclick="showBox('agency.do?method=gameAuthen&agencycode=${data.agencyCode}&areacode=${data.areaParent.code}','游戏授权',300,1000)">游戏授权</a></span> |
						<span><a href="#" onclick="showBox('agency.do?method=detailAgency&agencycode=${data.agencyCode}','详情',500,500)">详情</a></span> |
					...
					<c:if test="${data.agencyStatus.value == 1 && data.agencyType.value == 4}">
						...
						<span>清退</span> |
						<span>游戏授权</span> |
						<span><a href="#" onclick="showBox('agency.do?method=detailAgency&agencycode=${data.agencyCode}&agencyCodeTochar=${data.agencyCodeToChar}&agencyname=${data.agencyName}','详情',500,500)">详情</a></span> 
					</c:if>
					<c:if test="${data.agencyStatus.value == 2 && data.agencyType.value != 4}">
						...
						<span><a href="#" onclick="doclear('${data.agencyName}','agency.do?method=returnAgency&agencycode=${data.agencyCode}')" >清退</a></span> |								
						<span><a href="#" onclick="showBox('agency.do?method=gameAuthen&agencycode=${data.agencyCode}&agencyname=${data.agencyName}&areacode=${data.areaParent.code}','游戏授权',300,1000)">游戏授权</a></span> |
						<span><a href="#" onclick="showBox('agency.do?method=detailAgency&agencycode=${data.agencyCode}&agencyname=${data.agencyName}','详情',500,500)">详情</a></span> |
						<span><a href="#" onclick="showBox('terminal.do?method=addTerminal&agencycode=${data.agencyCode}&agencyCodeTochar=${data.agencyCodeToChar}&fatherArea=${data.agencyCode}&areaType=5','新增终端',400,700)"><img src="img/add-zdj.png" width="21" height="16" title="新增终端"/></a></span> |
