    ...
    <div class="queryDiv">
        <form action="item.do?method=listGoodsReceipts" method="POST" id="itemReceiptQueryForm">
            <div class="left">
           		...
           		<span>入库日期: <input id="receiptDate" name="receiptDate" value="${itemReceiptQueryForm.receiptDate}" class="Wdate text-normal" onfocus="WdatePicker({lang:'zh-cn',dateFmt:'yyyy-MM-dd',readOnly:true,isShowClear:true,isShowToday:true})" maxlength="40"/></span>
           		<span>入库仓库: 
        ...
        <table class="datatable">
        	<c:forEach var="data" items="${itemReceiptList}" varStatus="status">
            	<tr class="dataRow">
                	...
                	<td width="1%">&nbsp;</td>
                	<td style="width:*%">
                    	<span><a href="#" onclick="showBox('item.do?method=itemReceiptDetails&irNo=${data.irNo}&receiveWhName=${data.receiveWhName}&createAdminName=${data.createAdminName}&remark=${data.remark}','物品入库详情', 600, 800);">详情</a></span>&nbsp;|
                    	<span><a href="#" onclick="showPage('item.do?method=printItemReceiptSlip&irNo=${data.irNo}&receiveOrgName=${data.receiveOrgName}&receiveWhName=${data.receiveWhName}&remark=${data.remark}','打印物品入库单');">打印</a></span>
                	</td>
            	</tr>
            </c:forEach>
        </table>
