...
import cls.pilottery.oms.common.msg.UpdateSystemParameter11005Req;
		...
		if (fl != null) {
			...
			try {
				...
				if (gameManagementForm.getSysParam().getC_errcode() != 0) {
					...
					logger.debug("修改系统参数;");
					logger.debug("操作者["+currentUser.getId() +";"+JSONArray.toJSONString(strP));
					if (area.equals("sale")) {
						UpdateSystemParameter11005Req paramReq = new UpdateSystemParameter11005Req();
						paramReq.setPayCountLimit(new Long(gameManagementForm.getStr_sysParamValue2()));
						paramReq.setPayAmountLimit(new Long(gameManagementForm.getStr_sysParamValue3()));
						paramReq.setCancelCountLimit(new Long(gameManagementForm.getStr_sysParamValue0()));
						paramReq.setCancelAmountLimit(new Long(gameManagementForm.getStr_sysParamValue1()));
						sendMessageToHost(11005, paramReq);
					}
				}
			} catch (Exception e) {
				logger.error("errmsgs:" + e.getMessage());
