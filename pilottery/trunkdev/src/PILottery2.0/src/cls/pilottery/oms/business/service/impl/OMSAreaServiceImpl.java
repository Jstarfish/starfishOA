...
public class OMSAreaServiceImpl implements OMSAreaService {
	...
	public void updateGameAuth(OMSAreaAuthForm omsAreaAuthForm) throws Exception {
		...
		try {
			conn = DBConnectUtil.getOmsConnection();
			conn.setAutoCommit(false);
			stmt = conn.prepareCall("{ call p_om_area_auth(?,?,?) }");
			
			StructDescriptor sd = new StructDescriptor("TYPE_GAME_AUTH_INFO", conn);
			...
			ArrayDescriptor orc_table = ArrayDescriptor.createDescriptor("TYPE_GAME_AUTH_LIST", conn);
