...
public class XteaUtil {
	...
	private static long[] TEAEncrypt(long[] data, long[] key) {
		...
		long z = data[data.length - 1], y = data[0], sum = 0, e = 0, p = 0, q = 0;

		q = 6 + 52 / n;
		
		while (q-- > 0) {
			// System.out.println("z=" + z + " y=" + y + " sum=" + sum + " e=" +
			// e + " n=" + n + " p=" + p + " q=" + q);
			sum += DELTA;
			e = (sum >> 2) & 3;
			for (p = 0; p < n - 1; p++) {
				// System.out.println("z=" + z + " y=" + y + " sum=" + sum + "
				// e=" + e + " n=" + n + " p=" + p + " q=" + q);
				y = data[(int) (p + 1)];
	...
	public static void main(String[] args) {
		...
		String result = XteaUtil.Encrypt(reqJson, "12345678");
