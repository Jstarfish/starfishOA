...
public class ItemController {
    ...
    public Map<String, String> deleteItemType(HttpServletRequest request, ModelMap model, ItemTypeDeleteForm itemTypeDeleteForm) throws Exception
    {
        ...
        try {
            ...
            {
            	json.put("message", LocaleUtil.getUserLocaleErrorMsg(itemTypeDeleteForm.getC_errmsg(), request));
            }
        } catch (Exception e) {
            logger.error("errmsgs" + e.getMessage());
	...
	{	
		...
		{
			if (currentUser.getUserLang() == UserLanguage.ZH) {
				model.addAttribute("system_message", "物品列表存在重复物品。");
			} else {
		...
		try {
			...
			if (newItemReceiptForm.getC_errcode().intValue() == 0) {
				if (currentUser.getUserLang() == UserLanguage.ZH) {
					model.addAttribute("system_message", "编号为 <b>" + newItemReceiptForm.getReceiptCode() + "</b> 的物品入库记录已成功生成。");
				} else {
				...
				logger.error("errmsgs:" + LocaleUtil.getUserLocaleErrorMsg(newItemReceiptForm.getC_errmsg(), request));
				model.addAttribute("system_message", LocaleUtil.getUserLocaleErrorMsg(newItemReceiptForm.getC_errmsg(), request));
				return LocaleUtil.getUserLocalePath("common/errorTip", request);
			}
		} catch (Exception e) {
			logger.error("errmsgs: " + e.getMessage());
	...
	{	
		...
		try {
			...
			if (newItemIssueForm.getC_errcode().intValue() == 0) {
				if (currentUser.getUserLang() == UserLanguage.ZH) {
					model.addAttribute("system_message", "编号为 <b>" + newItemIssueForm.getIssueCode() + "</b> 的物品出库记录已成功生成。");
				} else {
				...
				logger.error("errmsgs:" + LocaleUtil.getUserLocaleErrorMsg(newItemIssueForm.getC_errmsg(), request));
				model.addAttribute("system_message", LocaleUtil.getUserLocaleErrorMsg(newItemIssueForm.getC_errmsg(), request));
				return LocaleUtil.getUserLocalePath("common/errorTip", request);
			}
		} catch (Exception e) {
			logger.error("errmsgs: " + e.getMessage());
	...
	public String addItemCheck(HttpServletRequest request, ModelMap model, @ModelAttribute("newItemCheckForm") NewItemCheckForm newItemCheckForm) throws Exception
	{
		...
		try {
			...
			if (newItemCheckForm.getC_errcode().intValue() == 0) {
				if (currentUser.getUserLang() == UserLanguage.ZH) {
					model.addAttribute("system_message", "编号为 <b>" + newItemCheckForm.getCheckCode() + "</b> 的物品盘点记录已成功生成。");
				} else {
				...
				logger.error("errmsgs:" + LocaleUtil.getUserLocaleErrorMsg(newItemCheckForm.getC_errmsg(), request));
				model.addAttribute("system_message", LocaleUtil.getUserLocaleErrorMsg(newItemCheckForm.getC_errmsg(), request));
				return LocaleUtil.getUserLocalePath("common/errorTip", request);
			}
		} catch (Exception e) {
			logger.error("errmsgs: " + e.getMessage());
	...
	public String processItemCheck(HttpServletRequest request, ModelMap model, @ModelAttribute("procItemCheckForm") ProcItemCheckForm procItemCheckForm) throws Exception
	{
		...
		try {
			...
			if (procItemCheckForm.getC_errcode().intValue() == 0) {
				if (currentUser.getUserLang() == UserLanguage.ZH) {
					model.addAttribute("system_message", "编号为 <b>" + procItemCheckForm.getCheckNo() + "</b> 的物品盘点已成功处理。");
				} else {
				...
				logger.error("errmsgs:" + LocaleUtil.getUserLocaleErrorMsg(procItemCheckForm.getC_errmsg(), request));
				model.addAttribute("system_message", LocaleUtil.getUserLocaleErrorMsg(procItemCheckForm.getC_errmsg(), request));
				return LocaleUtil.getUserLocalePath("common/errorTip", request);
			}
		} catch (Exception e) {
			logger.error("errmsgs: " + e.getMessage());
	...
	public String addItemDamage(HttpServletRequest request, ModelMap model, @ModelAttribute("newItemDamageForm") NewItemDamageForm newItemDamageForm) throws Exception
	{
		...
		try {
			...
			if (newItemDamageForm.getC_errcode().intValue() == 0) {
				if (currentUser.getUserLang() == UserLanguage.ZH) {
					model.addAttribute("system_message", "编号为 <b>" + newItemDamageForm.getIdNo() + "</b> 的物品损毁记录已成功生成。");
				} else {
					model.addAttribute("system_message", "Item damage record <b>" + newItemDamageForm.getIdNo() + "</b> has been successfully created.");
				}
				return LocaleUtil.getUserLocalePath("common/successTip", request);
			} else {
				model.addAttribute("system_message", LocaleUtil.getUserLocaleErrorMsg(newItemDamageForm.getC_errmsg(), request));
    			return LocaleUtil.getUserLocalePath("common/errorTip", request);
			}
		} catch (Exception e) {
			model.addAttribute("system_message", e.getMessage());
