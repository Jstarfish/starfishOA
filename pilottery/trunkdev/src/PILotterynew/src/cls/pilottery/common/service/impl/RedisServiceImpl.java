package cls.pilottery.common.service.impl;
...
import cls.pilottery.common.utils.SerializeUtils;

/**
 * 项目名：泰山无纸化平台 文件名：RedisServiceImpl.java 
 * 类描述：定义了Redis常用的操作
 * 创建人：huangchengyuan@chinalotsynergy.com 
 * 日期：</b>2015-8-20-下午03:52:42
 * 版本信息：v1.0.0 Copyright (c) 2015华彩控股有限公司-版权所有
 */
@Component
public class RedisServiceImpl implements RedisService {
	...
	public long expire(String key, int seconds) {
		...
		try {
			...
			log.error("EXPIRE error[key=" + key + " seconds=" + seconds + "]"
					+ ex.getMessage(), ex);
	...
	public long expireAt(String key, int unixTimestamp) {
		...
		try {
			...
			log.error("EXPIRE error[key=" + key + " unixTimestamp="
	...
	public String trimList(String key, long start, long end) {
		...
		try {
			...
			log.error("LTRIM 出错[key=" + key + " start=" + start + " end=" + end
					+ "]" + ex.getMessage(), ex);
			returnBrokenResource(shardedJedis);
		} finally {
			returnResource(shardedJedis);
	...
	public List<String> rangeList(String key, long start, long end) {
		...
		try {
			...
			log.error("rangeList 出错[key=" + key + " start=" + start + " end="
