    ...
    <div class="queryDiv">
        <form action="item.do?method=listGoodsIssues" method="POST" id="itemIssueQueryForm">
            <div class="left">
				...
				<span>出库日期: <input id="issueDate" name="issueDate" value="${itemIssueQueryForm.issueDate}" class="Wdate text-normal" onfocus="WdatePicker({lang:'zh-cn',dateFmt:'yyyy-MM-dd',readOnly:true,isShowClear:true,isShowToday:true})" maxlength="40"/></span>
				<span>出库仓库:
        ...
        <table class="datatable">
        	<c:forEach var="data" items="${itemIssueList}" varStatus="status">
        		<tr class="dataRow">
        			...
        			<td width="1%">&nbsp;</td>
        			<td style="width:*%">
	                    <span><a href="#" onclick="showBox('item.do?method=itemIssueDetails&iiNo=${data.iiNo}&operAdminName=${data.operAdminName}&receiveOrgName=${data.receiveOrgName}&sendOrgName=${data.sendOrgName}&sendWhName=${data.sendWhName}&remark=${data.remark}','物品出库详情', 600, 800);">详情</a></span>&nbsp;|
	                    <span><a href="#" onclick="showPage('item.do?method=printItemIssueSlip&iiNo=${data.iiNo}&receiveOrgName=${data.receiveOrgName}&sendOrgName=${data.sendOrgName}&sendWhName=${data.sendWhName}&remark=${data.remark}','打印物品出库单');">打印</a></span>
                	</td>
        		</tr>
        	</c:forEach>
        </table>
