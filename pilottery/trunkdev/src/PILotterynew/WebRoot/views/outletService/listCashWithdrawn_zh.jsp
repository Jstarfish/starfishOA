    ...
    <div id="bodyDiv">
        ...
        <c:forEach var="obj" items="${pageDataList}" varStatus="status" >
            <tr class="dataRow">
            	...
            	<td width="1%">&nbsp;</td>
                 <td style="width:8%">
                	<c:if test="${obj.applyStatus==1 }"> 已提交 </c:if>
					<c:if test="${obj.applyStatus==2 }"> 已取消 </c:if> 
					<c:if test="${obj.applyStatus==3 }"> 已审批</c:if>
					<c:if test="${obj.applyStatus==4 }"> 已拒绝</c:if> 
					<c:if test="${obj.applyStatus==5 }"> 已提现</c:if>
					<c:if test="${obj.applyStatus==6 }"> 已提现</c:if>
                </td> 
