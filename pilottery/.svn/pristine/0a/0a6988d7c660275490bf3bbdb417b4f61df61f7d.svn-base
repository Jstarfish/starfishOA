...
<script type="text/javascript">
	...
	function query(){
		...
		if (Number(issueNumberStart) > Number(issueNumberEnd)) {
			showMsg('Message','Beginning issue can not be ',"warn");
			return;
		}
		$("#queryForm").submit();
	}
...
<body>
	<div id="title">Abandoned Awards Inquiry</div>
	...
	<div class="queryDiv">
        <form:form modelAttribute="abandonAward" action="abandon.do?method=abandonAward" id="queryForm">
            <div class="left">                                    
				...
				<!-- 查询 -->
				<input type="button" value="Query" onclick="query();" class="button-normal"></input>
            </div>
        </form:form>
    </div>
        ...
        <table class="datatable" id="exportPdfData">
			...
			<tr class="dataRow">
				...
				<c:if test="${data.allBetCount!=0 && data.allTicketCount!=0}">
					<span><a href="#" onclick="showBox('abandon.do?method=detailAbandon&payDate=${data.payDate}&gameCode=${data.gameCode}&issueNumber=${data.issueNumber}','Abandon Details',360,603)" 
						rev="Abandon Details">detail</a></span>
				</c:if>
				<c:if test="${data.allBetCount==0 || data.allTicketCount==0}">
					<span>detail</span>
				</c:if>
