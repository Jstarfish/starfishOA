	...
	function initSelect(value) {
		...
		$.ajax({
			...
			success : function(r) {
				$("#batchNoQuery").empty();
				if(value == ''){
					$("#batchNoQuery").append("<option	value=''>All</option>");
				}
			...
			<c:forEach var="items" items="${pageDataList }" varStatus="status">
				<tr class="dataRow">
					...
					<td style="width: *%"><span><a href="#"
							onclick="showBox('damagedGoods.do?method=detailsDamaged&record=${items.recordCode }','Damaged Details',550,980)">Details</a></span>
					</td>
				</tr>
			</c:forEach>
		</table>
