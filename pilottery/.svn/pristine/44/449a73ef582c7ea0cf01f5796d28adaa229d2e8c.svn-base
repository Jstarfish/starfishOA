...
function approve(stbNo){
	showBoxRefresh('transfer.do?method=initAproveStockTransfer&stbNo=' + stbNo,'Stock Transfer Approve',400,800);
}
function expired(stbNo) {
	var url = "transfer.do?method=expiredStockTransfer&stbNo=" + stbNo;
	showDialog("doExpired('"+url+"')",'Expired','Are you sure you want to expired<br/> the stock transfer: '+stbNo);
}
                ...
                <td style="width:*%">
                    ...
                    <span>
                    	<c:if test="${obj.status == 7 && sessionScope.isCollector != 1}">
                    		<a href="#" onclick="expired('${obj.stbNo}')">Expired</a>
                    	</c:if>
                    	<c:if test="${obj.status != 7 || sessionScope.isCollector == 1}">
                    		Expired
                    	</c:if>
