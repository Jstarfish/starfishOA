        ...
        <tr class="dataRow">
        	...
        	<td width="330">
                ...
                <c:when test="${row.isPublish==0 && row.issueStatus==0 && (row.gameCode==6 || row.gameCode==7)}">
               		<a href="#" onclick="showBox('issueManagement.do?method=editCalcWinningParam&gameCode=${row.gameCode}&issueNumber=${row.issueNumber}','Calc Param',400,550)">Calc Param</a> |
                </c:when>
                <c:when test="${row.issueStatus==13}">
                </c:when>
                <c:otherwise>
