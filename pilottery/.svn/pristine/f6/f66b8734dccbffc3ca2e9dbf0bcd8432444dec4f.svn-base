...
function getWhByInst(){
	...
	$('#houseCode').empty();
	if(orgCode == ''){
		$('#houseCode').append("<option value=''>--All--</option>"); 
		return false;
	}
...
<body>
    <!-- 顶部标题块 -->
    <div id="title">Inventory Checks Inquiry</div>
        ...
        <c:forEach var="data" items="${pageDataList}" varStatus="status" >
            <tr class="dataRow">
				...
				<td style="width: *%"><span><a href="#"
					onclick="showBox('inventory.do?method=getCheckPointDetail&cpNo=${data.cpNo}','Checked Details', 550, 980);">Details</a></span>
				</td>
			</tr>
        </c:forEach>
        </table>
