...
int get_sequence(uint64 *sequence)
{
    char str[20] = { 0 };
    memset(str, 0, 20);
    FILE *fp = fopen("./test_seq.txt", "r+");//文件必须存在
    if (fp == NULL) {
...
//        header->inm_header.status = SYS_RESULT_T_AGENCY_ERR;//ap不可用
//        return len;
//    }
...
//    }
    ...
    if (strlen(json_bet_string->valuestring) >= 2048) {
        header->inm_header.status = SYS_RESULT_SELL_DATA_ERR; //投注字符串格式错误
        log_error("bet_string len [%d]", strlen(json_bet_string->valuestring));
        return len;
    }
    ...
    //        return len;
    //    }
        //fbs_inm_msg->issueNumber = curr_issue_info->;           //系统处理此票时的当前期号
        //fbs_inm_msg-> = start_issue_info->issueNumber;         //销售票实际真实购买的第一期的期号
        //fbs_inm_msg->ticket.issueSeq = start_issue_info->serialNumber;     //销售票实际真实购买的第一期的期次序号
        //fbs_inm_msg->ticket.lastIssue = end_issue_info->issueNumber;       //销售票实际真实购买的最后一期的期号
        //fbs_inm_msg->drawTimeStamp = end_issue_info->drawTime;             //最后一期的开奖时间
...
int r_oms_inquiryticket(char *inm_buf, GIDB_TICKET_IDX_REC *tidx_rec)
{
    ...
    inm_msg->startIssueNumber = tidx_rec->issueNumber; //销售期号
    ...
    if (ret < 0) {
        ...
        msg_header->inm_header.status = OMS_RESULT_TICKET_NOT_FOUND_ERR; //没有找到这张彩票
        return len;
    }
    ...
    if (inm_msg->issueCount > 1) {
        ...
        if (ISSUE_STATE_ISSUE_END != last_issue_info.status) {
            inm_msg->isWin = 0;
            msg_header->inm_header.status = OMS_RESULT_SUCCESS; //未开奖完成
            return len;
        }
    }
    if (ISSUE_STATE_ISSUE_END != issue_info.status) {
        ...
        msg_header->inm_header.status = OMS_RESULT_SUCCESS; //期还没有开奖
        return len;
    }
    //查询此彩票是否中奖
    ...
    if (ret < 0) {
        ...
        inm_msg->isWin = 1;
        msg_header->inm_header.status = OMS_RESULT_SUCCESS; //未中奖
        return len;
    }
    // 中奖
    inm_msg->isWin = 2;
...
int r_oms_fbs_inquiryticket(char *inm_buf, GIDB_TICKET_IDX_REC *tidx_rec)
{
    ...
    strcpy(inm_msg->rspfn_ticket, tidx_rec->rspfn_ticket);
    inm_msg->gameCode = tidx_rec->gameCode;
    inm_msg->issue_number = tidx_rec->issueNumber; //销售期号
...
int R_OMS_ticket_inquiry_process(NCPC_SERVER *ns, cJSON *json_params, char *inm_buf)
{
    ...
    if (unique_tsn == 0) {
        ...
        msg_header->inm_header.status = OMS_RESULT_TICKET_TSN_ERR; //TSN错误
        return len;
    }
    //从索引文件中查询票信息
    char tidx_buf[512] = { 0 };
...
int s_oms_fbs_inquiryticket(char *inm_buf, cJSON *json_result)
{
    ...
    //    cJSON_AddNumberToObject(json_result, "sub_type", inm_msg->sub_type);//OMS不要,从betString解析
...
int r_oms_payticket(cJSON *json_params, char *inm_buf, GIDB_TICKET_IDX_REC *tidx_rec)
{
    ...
    inm_msg->issueNumber_pay = 0; //caoxf__ 此字段暂时保留
    ...
    if (!transctrlParam->payFlag) {
        ...
        msg_header->inm_header.status = OMS_RESULT_PAY_DISABLE_ERR; //此游戏不可兑奖
        return len;
    }
    //检查是否在服务时段的范围内
    if (false == gl_verifyServiceTime(game_code)) {
        log_debug("OMS_RESULT_GAME_SERVICETIME_OUT_ERR: game[%d] time out of service time.", game_code);
        msg_header->inm_header.status = OMS_RESULT_GAME_SERVICETIME_OUT_ERR; //当前不处于彩票交易时段
        return len;
    }
    //check 最后一期的期次状态
    GIDB_ISSUE_HANDLE * i_handle = gidb_i_get_handle(game_code);
    ...
    if (ISSUE_STATE_ISSUE_END != last_issue_info.status) {
        if (last_issue_info.status < ISSUE_STATE_CLOSED) {
            ...
            msg_header->inm_header.status = OMS_RESULT_PAY_NOT_DRAW_ERR; //期还没有开奖
            return len;
        }
        else {
            log_debug("issue[%lld] state[%d] >= ISSUE_STATE_CLOSED. tsn[%s]",
                last_issue_info.issueNumber, last_issue_info.status, inm_msg->rspfn_ticket);
            msg_header->inm_header.status = OMS_RESULT_PAY_WAIT_DRAW_ERR; //期次等待开奖完成
            return len;
        }
    }
    //检查期次相关
    ...
    if (ret < 0) {
        ...
        msg_header->inm_header.status = OMS_RESULT_PAY_NOT_WIN_ERR; //彩票未中奖
        return len;
    }
    //检查兑奖期
    ...
    if (1 == inm_msg->isTrain) {
        ...
        msg_header->inm_header.status = OMS_RESULT_PAY_TRAINING_TICKET_ERR; //不可在OM兑培训票
        return len;
    }
    // 中奖
    if (pWinTicket->paid_status == PRIZE_PAYMENT_NONE) {
        ...
        msg_header->inm_header.status = OMS_RESULT_PAY_MULTI_ISSUE_ERR; //多期票未完结   *************不太可能走到这里，前面已经校验过所有期次已期结*********
        return len;
    }
        ...
        msg_header->inm_header.status = OMS_RESULT_PAY_PAID_ERR; //已兑奖
        return len;
    }
    //判断游戏兑奖保护限额
    if (pWinTicket->winningAmountWithTax >= transctrlParam->gamePayLimited) {
        ...
        msg_header->inm_header.status = OMS_RESULT_PAY_MONEY_LIMIT_ERR; //超出游戏兑奖保护金额
        return len;
    }
    //检查部门兑奖限额
    /*
...
int r_oms_fbs_payticket(cJSON *json_params, char *inm_buf, GIDB_TICKET_IDX_REC *tidx_rec)
{
    ...
    if (0 == flagEnd) {
        ...
        if (inm_msg->matchCount == 1) {
            ...
            msg_header->inm_header.status = OMS_RESULT_PAY_MULTI_ISSUE_ERR;//多场比赛还没有开完奖
            return len;
        }
    }
...
int r_oms_cancelticket(cJSON *json_params, char *inm_buf, GIDB_TICKET_IDX_REC *tidx_rec)
{
    ...
    if (!transctrlParam->cancelFlag) {
        ...
        msg_header->inm_header.status = OMS_RESULT_CANCEL_DISABLE_ERR; //此游戏不可退票
        return len;
    }
    //检查是否在服务时段的范围内
    if (false == gl_verifyServiceTime(game_code)) {
    ...
    if (issue_info == NULL) {
        ...
        msg_header->inm_header.status = OMS_RESULT_CANCEL_ISSUE_ERR; //没有当前期，不可以退票
        return len;
    }
    if (inm_msg->saleStartIssue != issue_info->issueNumber) {
        ...
        msg_header->inm_header.status = OMS_RESULT_CANCEL_ISSUE_ERR; //当前期不是销售期，不可以退票
        return len;
    }
    if (issue_info->curState > ISSUE_STATE_CLOSING) {
        log_info("OMS_RESULT_CANCEL_ISSUE_ERR: current issue has already closed. tsn[%s] game[%d] issueNumber[%lld]",
            inm_msg->rspfn_ticket, game_code, issue_info->issueNumber);
        msg_header->inm_header.status = OMS_RESULT_CANCEL_ISSUE_ERR; //当前期次已关闭
        return len;
    }
    ...
    if (pSaleTicket->isCancel) {
        ...
        msg_header->inm_header.status = OMS_RESULT_CANCEL_AGAIN_ERR; //已退票
        return len;
    }
    //检查是否是培训票
    if (1 == pSaleTicket->isTrain) {
        log_debug("OMS_RESULT_T_CANCEL_TRAINING_TICKET_ERR: tsn[ %s ]", inm_msg->rspfn_ticket);
        msg_header->inm_header.status = OMS_RESULT_CANCEL_TRAINING_TICKET_ERR; //不可在OM退培训票
        return len;
    }
    //检查部门退票限额
    /*
    if (inm_msg->cancelAmount > transctrlParam->branchCenterCancelLimited) {
        ...
        msg_header->inm_header.status = OMS_RESULT_CANCEL_MONEY_LIMIT_ERR; //超出退票保护金额
        return len;
    }
    */
    //退票权限的判断，目前OMS(包括部门的OMS) 默认可以退所有票
...
int r_oms_fbs_cancelticket(cJSON *json_params, char *inm_buf, GIDB_TICKET_IDX_REC *tidx_rec)
{
    ...
    for (int i = 0; i < inm_msg->matchCount; i++) {
        ...
        if (match == NULL) {
            ...
            msg_header->inm_header.status = OMS_RESUTL_CANCEL_NOT_ACCEPT; //场次已有关闭
            return len;
        }
        matchCode++;
    }
...
int R_OMS_fbs_draw_input_result_process(NCPC_SERVER *ns, cJSON *json_params, char *inm_buf)
{
    ...
    for (i = 1; ; i++) {
        ...
        //strcpy((char*)(inm_msg->drawResults+i), atoi(strtrim(p+2)));//跳过'x:'
        inm_msg->drawResults[i] = atoi(strtrim(p + 2));//跳过'x:'
        p = strtok_r(NULL, ",", &last);
    }
    ...
    for (i = 0; ; i++) {
        ...
        //strcpy((char*)(inm_msg->matchResult+i), strtrim(p+2));//跳过'x:'
        inm_msg->matchResult[i] = atoi(strtrim(p + 2));//跳过'x:'
        p = strtok_r(NULL, ",", &last);
    }
    if (i != 8) {
    ...
    //向开奖日志表写入一条记录，通知gl_draw_fbs任务，此场次可以进行开奖
