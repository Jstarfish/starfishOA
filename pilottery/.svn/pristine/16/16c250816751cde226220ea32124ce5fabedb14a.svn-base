            ...
            <table width="1000" border="0" cellspacing="0" cellpadding="0" align="center" class="normal">
                <tr>
                    <td colspan="6" align="center"
                        style="font-size:24px;text-align: center; line-height:50px;border-bottom: 2px solid #000;">兑奖领取单
                    </td>
                </tr>
                <tr height="40">
                    <td colspan="3">
                        <table width="100%" border="0" cellspacing="0" cellpadding="0">
                            <tr>
                                <td height="40">兑奖记录编号：${id}</td>
                                <td align="right" height="40">兑奖日期：${currdatetime}</td>
                            </tr>
                            <tr>
                                <td height="40">经办人：${operator}</td>
                            </tr>
                            <tr>
                                <td height="40">单位：瑞尔</td>
                            </tr>
                        </table>
                    </td>
                </tr>
                        ...
                        <table width="1000" border="1" align="center" cellpadding="0" cellspacing="0" class="normal">
                            ...
                            <c:if test="${not empty lotteryReq.prizeDetail}">
                                <c:forEach var="prize" items="${lotteryReq.prizeDetail}">
                                    <tr align="center" height="80">
                                        <c:if test="${expirydateForm.gameCode!=14}">
                                            <td>${prize.name}</td>
                                        </c:if>
                                        <c:if test="${expirydateForm.gameCode==14}">
                                            <td>-</td>
                                        ...
                                        <td>
                                            <c:if test="${expirydateForm.gameCode!=14}">
                                                <fmt:formatNumber value="${prize.amountSingle}" pattern="#,###"/>
                                            </c:if>
                                            <c:if test="${expirydateForm.gameCode==14}">
                                                -
                                            </c:if>
