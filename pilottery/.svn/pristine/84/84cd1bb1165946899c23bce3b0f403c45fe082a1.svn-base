...
function prompt(d) {
	var msg = "你确定要修改?";
	showDialog("updateGameParameter("+d+")","确认",msg);
}
...
function doChange(btnId, flgId) {
	var msg = "你确定要修改?";
	showDialog("on_off('"+btnId+"','"+flgId+"')","确认",msg);
}
            ...
            <td><div style="height:80px;margin-top:20px;margin-left:2%;margin-right:6%;background:lightgray;" >
                ...
                <table width="100%" border="0" cellspacing="0" cellpadding="0" style="height:70px;">
                    ...
                    <tr>
                       ...
                       <td>当前期次状态：${issueStatusValue}</td>
                    </tr>
                    <tr>
                       <td>期次结束时间：<c:if test="${gameManagementForm.gameIssue.realCloseTime!=null}">
                       					<fmt:formatDate value="${gameManagementForm.gameIssue.realCloseTime}" pattern="yyyy-MM-dd HH:mm:ss"/>
                       				</c:if>
		                     		<c:if test="${gameManagementForm.gameIssue.realCloseTime==null}">
		                     			<fmt:formatDate value="${gameManagementForm.gameIssue.planCloseTime}" pattern="yyyy-MM-dd HH:mm:ss"/>
		                     		</c:if>
                       </td>
