...
<script type="text/javascript" charset="UTF-8">
	...
	function delte(outletCode) {
		...
		$.ajax({
				...
				success : function(r) {
					if(r=='1'){
						showError("The outlet banlance not 0 ,can not be deleted");
					}else if (r == '2') {
						showError("The outlet has deals,you need to clear it");
					} else {
						showDialog("deleteOutlet('" + outletCode + "')","Delete", msg);
					}
				}
			});
...
<body>
	<!-- 顶部标题块 -->
	<div id="title">Outlet Management</div>
	<!-- 查询条件块 -->
	<div class="queryDiv">
			...
			<c:forEach var="items" items="${listOutlets }" varStatus="status">
				<tr class="dataRow">
					...
					<td style="width: *%"><c:if test="${items.status==1 }">
							<span><a href="#" onclick="resetPassword('${items.outletCode }')">Reset Password</a></span>&nbsp;|
							<span><a href="#"
						...
						<span><a href="#"
							...
							<span style="color: gray">Reset Password</span>&nbsp;|
							<span><a href="#"
								onclick="showBox('outlets.do?method=detailsOutlets&outletCode=${items.outletCode }','Outlet Details',450,800)">Details</a></span>&nbsp;|
