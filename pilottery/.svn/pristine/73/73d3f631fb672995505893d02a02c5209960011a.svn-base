	            ...
	            <tr>
					<c:if test="${agencyCode!=null}">
						...
						<td align="center" width="35%">
							<input class="text-big" style="border: none;" disabled="disabled" type="text" value="${terminal.agencyParent.name }"/>
						</td>
						<td width="*%">
							<input type="hidden" name="agencyParent.code" value="${terminal.agencyParent.code }"/>
							<input type="hidden" name="flag" value="nonrefresh"/><!-- 不刷新 -->
						</td>
					</c:if>
					<c:if test="${agencyCode==null}">
						<td align="right" width="25%">Agency Code：</td>
