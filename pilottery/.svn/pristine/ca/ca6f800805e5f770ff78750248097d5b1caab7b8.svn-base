...
public class GameManagementController {
	...
	private BaseMessageRes sendMessageToHost(int func, Object params) {
	...
	public String updateGameParameter(HttpServletRequest request, HttpServletResponse response, ModelMap model,
		...
		if (gameCode_ != null) {  //取消编辑的情况，form提交时带个gameCode参数
			...
			try {
				...
				BaseMessageRes res = sendMessageToHost(2003, paramReq);
	...
	public String updateGameControlParameter(HttpServletRequest request, HttpServletResponse response, ModelMap model,
		...
		if (gameCode_ != null){	//取消编辑的情况，form提交时带个gameCode参数
			...
			try {
				...
				BaseMessageRes res = sendMessageToHost(2005, paramReq);
				if(res != null && res.getRc() == 0){
	...
	public String editRiskControl(HttpServletRequest request, HttpServletResponse response, ModelMap model,
		...
		if ((gameParameter.getGameCode() == SysConstants.KOC11X5 || gameParameter.getGameCode() == SysConstants.TEMA) && riskParam != null) { //11选5
			String[] temp = riskParam.split(":");
			RiskControlVo11X5 vo = new RiskControlVo11X5();
			vo.setParamA(temp[0]);
			vo.setParamB(temp[1]);
	...
	public String updateRiskControl(HttpServletRequest request, HttpServletResponse response, ModelMap model,
		...
		if (gameCode_ != null) {	//取消编辑的情况，form提交时带个gameCode参数
			...
			String riskParam = gameParameter.getRiskParam();
			if ((gameParameter.getGameCode() == SysConstants.KOC11X5 || gameParameter.getGameCode() == SysConstants.TEMA) && riskParam != null && !riskParam.equals("")) {
				String[] temp = riskParam.split(":");
				RiskControlVo11X5 vo = new RiskControlVo11X5();
				vo.setParamA(temp[0]);
				vo.setParamB(temp[1]);
				...
				vo.setParamB(temp[1]);
				model.addAttribute("riskControlK3", vo);
			}else if (gameParameter.getGameCode() == SysConstants.KOCSSC && riskParam != null && !riskParam.equals("")) {
			...
			try {
				...
				String riskParamStr = "";
				if (gph.getGameCode() == SysConstants.KOC11X5 || gph.getGameCode() == SysConstants.TEMA ) {
					RiskControlVo11X5 vo = new RiskControlVo11X5();
					...
					//获取风控参数
					GameParameterHistory gameParameter = gameManagementService.getGameParameterHistory(SysConstants.KOCSSC);
					String riskParam = gameParameter.getRiskParam();
					String[] risk = riskParam.split(";");
					String returnRate =  risk[0];
					//riskParam = riskParam.substring(0, riskParam.indexOf(";"));
					//String[] temp = riskParam.split(",");
					...
					//在字符串前自增固定的字符串
					riskParamStr += returnRate+";";
					for (int i = 0; i < size; i++) {
						String pa = request.getParameter("gameParam"+i+".paramA");
						String pb = request.getParameter("gameParam"+i+".paramB");
						riskParamStr += pa+":"+pb+",";
						RiskControlVoSSC vo = new RiskControlVoSSC();
						vo.setParamA(pa);
						vo.setParamB(pb);
				...
				BaseMessageRes res = sendMessageToHost(2009, paramReq);
				if(res !=null && res.getRc() == 0){
		...
		if (gameCode_ != null) {	//取消编辑的情况，form提交时带个gameCode参数
			...
			if (d == 2) {
				...
				try {
					...
					BaseMessageRes res = sendMessageToHost(2021, paramReq);
					if(res !=null && res.getRc() == 0){
				...
				try {
					...
					BaseMessageRes res = sendMessageToHost(2019, paramReq);
					if(res !=null && res.getRc() == 0){
	...
	public void changeStatus(HttpServletRequest request, HttpServletResponse response,
		...
		if (isSale != null) {
			...
			try {
				...
				BaseMessageRes res = sendMessageToHost(2011, paramReq);
				if(res !=null && res.getRc() == 0){
		...
		if (isPay != null) {
			...
			try {
				...
				BaseMessageRes res = sendMessageToHost(2013, paramReq);
				if(res !=null && res.getRc() == 0){
					gameManagementService.changeParamStatus(gp);
				}
			} catch (Exception e) {
		...
		if (isCancel != null) {
			...
			try {
				...
				BaseMessageRes res = sendMessageToHost(2015, paramReq);
				if(res !=null && res.getRc() == 0){
					gameManagementService.changeParamStatus(gp);
				}
			} catch (Exception e) {
		...
		if (isAutoDraw != null) {
			...
			try {
				...
				BaseMessageRes res = sendMessageToHost(2017, paramReq);
				if(res !=null && res.getRc() == 0){
					gameManagementService.changeParamStatus(gp);
				}
			} catch (Exception e) {
