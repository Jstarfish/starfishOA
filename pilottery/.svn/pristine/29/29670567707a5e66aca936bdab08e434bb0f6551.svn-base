...
function showMsg() {
    ...
    function checkDrawNumber(){
        ...
        var regex1;
        var errMsg = '* Incorrect draw numbers format';
        ...
        if(6 == findObj("gameCode").value) {
        	...
        	for(var i in arr){
        		...
        		if(str.indexOf(arr[i]) != str.lastIndexOf(arr[i])) {
        			regex1 = false;
        			errMsg = '* No repeatition for draw numbers';
        			break;
        	    } else if(arr[i] > 40 || arr[i] < 1){
        	        regex1 = false;
                    errMsg = '* Dragon Star 7 draw number range: 1 - 40!';
                    break;
            	}
        	}
