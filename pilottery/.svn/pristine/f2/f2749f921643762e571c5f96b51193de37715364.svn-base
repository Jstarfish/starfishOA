...
import cls.pilottery.webncp.system.dao.VersionDao;
...
import cls.pilottery.webncp.system.model.Request8005Model;
...
public class TerminalVersionServiceImpl implements TerminalVersionService {
	...
	@Override
	@WebncpMethod(code="8005")
	public BaseResponse checkTerminal(String reqJson) throws Exception{
		BaseResponse res = new BaseResponse();
		Request8005Model req = JSONObject.parseObject(reqJson,Request8005Model.class);
		if(req == null || StringUtils.isEmpty(req.getLoginPwd()) || StringUtils.isEmpty(req.toString())){
		...
		String password = this.versionDao.getUserPwdById(req.getCollectorid());
		if(password == null){
			res.setErrorCode(800401);
			log.info("用户不存在");
			return res;
		}else{
			if(MD5Util.MD5Encode(req.getLoginPwd()).equals(password)){
				long currentBalance = this.versionDao.getCurrentBalance(req.getTerminalCode());
				Map<String,Object> map = new HashMap<String,Object>();
				map.put("outletCode", req.getTerminalCode());
				map.put("userId", req.getCollectorid());
				map.put("checkBalance", req.getCheckBalance());
				map.put("checkTerminal", req.getCheckTerminal());
				map.put("agencyBalance", currentBalance);
				
				this.versionDao.saveCheckInfo(map);
			}else{
				res.setErrorCode(800402);
				log.info("登陆密码不正确");
				return res;
			}
		}
