...
public class GameAuth implements Comparable<GameAuth>, java.io.Serializable{
	...
	private String     areaCode;			//授权区域
	
	private transient String	 agencyCode;        	//授权销售站
	private long	 gameCode;				//游戏编号
	private transient String	 name;					//游戏名称
	private transient int		 type = 1;          	//游戏类型（1=基诺,2=乐透,3=数字）
	private int  	 status = 0;      	    //是否可用（1=可用，0=不可用）
	private transient int      fee = 0;				//发行费比例
	private int		 payStatus = 0;     	//（1=是；0=否）
	private int		 sellStatus = 0;    	//是否可销售
	private int		 cancelStatus = 0;			//是否可退票
