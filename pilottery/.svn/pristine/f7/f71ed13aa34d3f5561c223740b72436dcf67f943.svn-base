		 	...
		 	<tr class="headRow">
			  ...
			  <td width="*%">Payout Scope</td>
    			...
    			<tr class="dataRow">
						...
						<input type="hidden" name="guth[${s.index}].areaCode" value="${game.areaCode}" />
						</td>
						<td>
						...
						<c:if test="${game.flag==0}">
							...
							<c:when test="${game.claimingScope == 1}">
								<select name="guth[${s.index}].claimingScope" class="select-s">					  					
				  					...
				  					<option value="1" selected="selected">Institution</option>
				  					<option value="4">Outlet</option>
								</select>
							</c:when>
							<c:when test="${game.claimingScope == 4}">
								<select name="guth[${s.index}].claimingScope" class="select-s">					  					
				  					<option value ="0" >Center</option>
				  					<option value="1">Institution</option>
				  					<option value="4" selected="selected">Outlet</option>
								</select>
							</c:when>
							<c:otherwise>
								<select name="guth[${s.index}].claimingScope" class="select-s">					  					
