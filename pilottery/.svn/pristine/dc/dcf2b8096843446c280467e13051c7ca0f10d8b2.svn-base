			...
			<div class="left">
            	<span>Level:
            		<select id="eventLevel" name="eventLevel" class="select-normal">
						...
						<option value="1" <c:if test="${form.eventLevel==1}">selected="selected"</c:if>>info</option>
			...
			<c:forEach var="data" items="${pageDataList}" varStatus="status">
				...
				>	
					...
					<td style="width: 12%" title="${data.eventLevel}">
						 <c:choose>
				            <c:when test="${data.eventLevel == 1}">info</c:when>
				            <c:when test="${data.eventLevel == 2}">warn</c:when>
				            <c:when test="${data.eventLevel == 3}">error</c:when>
         					   <c:otherwise>fetal</c:otherwise>
						</c:choose> 
						</td>
					<td width="1%">&nbsp;</td>
