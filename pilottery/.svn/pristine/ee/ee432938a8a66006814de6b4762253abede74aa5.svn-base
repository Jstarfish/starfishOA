			...
			<tr>
				<td class="nr">
					...
					<table width="100%" border="0" cellspacing="0" cellpadding="0" style="font-size:14px; line-height:30px;">
						...
						<c:forEach items="${riskControlListSSC}" var="row" varStatus="s">
						<tr>
							<td>${ruleMap[row.paramA]}</td><input type="hidden" name="gameParam${s.index}.paramA" value="${row.paramA}"/>
							<td><input type="text" name="gameParam${s.index}.paramB" value="${row.paramB}" onblur="value=value.replace(/[^0-9]/g,'')" style="width:80px;"/></td>
						</tr>
						</c:forEach>
						</c:if>
						
						<c:if test="${gameParameter.gameCode==12 or gameParameter.gameCode==13}">
						<tr>
							<td>风控阈值</td>
