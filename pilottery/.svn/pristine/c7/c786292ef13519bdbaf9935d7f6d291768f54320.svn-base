...
public class MonitorServiceImpl implements MonitorService {
	...
	@Override
	public Map<String, Object> getSalePerByYear(String year, String insCode,String tjType) {
		...
		List<List<Long>> dayInAmount = new ArrayList<List<Long>>();// 天收入的收入集合
		// 获取每月的数据
		List<SalesStatisticsYearModel> monthData = monitorDao.getMonthcategories(new YearMonthForm(year, insCode,null,tjType));
		List<String> monthName = new ArrayList<String>();// 月的名称集合
		List<Map<String, Long>> monthAmount = new ArrayList<Map<String, Long>>();// 月的销售集合
		List<String> monthInName = new ArrayList<String>();// 月收入的名称集合
		List<Map<String, Long>> monthInAmount = new ArrayList<Map<String, Long>>();// 月收入的收入集合
		for (SalesStatisticsYearModel month : monthData) {
			...
			monthInName.add(month.getMonth());
			Map<String, Long> item = new HashMap<String, Long>();
			Map<String, Long> itemIn = new HashMap<String, Long>();
			item.put("y", month.getAmount());
			itemIn.put("y", month.getInComing());
			monthAmount.add(item);
			monthInAmount.add(itemIn);
			// 获取当月下每日的数据
			List<SalesStatisticsDayModel> dayData = monitorDao.getDayofAmount(new YearMonthForm(year, insCode,month.getMonth(),tjType));
			List<String> name = new ArrayList<String>();
			List<Long> amount = new ArrayList<Long>();
			List<Long> amountIn = new ArrayList<Long>();
			for (SalesStatisticsDayModel day : dayData) {
				name.add(day.getDay());
				amount.add(day.getAmount());
				amountIn.add(day.getInComing());
			}
			dayName.add(name);
			dayAmount.add(amount);
			dayInAmount.add(amountIn);
		}
		for (String month : monthName) {
			month = String.valueOf(Integer.valueOf(month) + 1);
		}
		for (String month : monthInName) {
			month = String.valueOf(Integer.valueOf(month) + 1);
		}
		resultList.put("monthName", monthName);
		resultList.put("monthNameIn", monthInName);
		resultList.put("monthAmount", monthAmount);
		resultList.put("monthAmountIn", monthInAmount);
		resultList.put("dayName", dayName);
		resultList.put("dayAmountIn", dayInAmount);
	...
	@Override
	public List<Plan> getInfPlanAndGamesList() {
		return monitorDao.getInfPlanAndGamesList();
	}

	@Override
	public List<HistoryRankingModel> getHistoryRankingsList(HistoryForm form) {
		return monitorDao.getHistoryRankingsList(form);
	}

	@Override
	public Integer getHistoryRankingsCount(HistoryForm form) {
		return monitorDao.getHistoryRankingsCount(form);
	}
}
