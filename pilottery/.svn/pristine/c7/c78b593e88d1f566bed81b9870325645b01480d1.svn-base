...
function delet(roleId){
	var msg = "你确定要删除该角色? ?";
	showDialog("deleteRole('"+roleId+"')","删除",msg);
	//deleteRole();
}
	 ...
	 <c:forEach var="u" items="${pageDataList}" varStatus="status" > 
		<tr class="dataRow">
			...
			<td style="width:*%" scope="col">
				...
				|
				<a href="#" onclick="showBox('role.do?method=addRoleUser&roleId=${u.id}', '添加用户',550,820);">添加用户</a>
				|
				<a href="#" onclick="showBox('role.do?method=rolePermissions&roleId=${u.id}', '角色权限',550,800);">权限</a>
				|
