...
uint16  rk_get2StarDirectNumberIndex(BETLINE *line, uint16 dindex[])
{
    ...
    switch(GL_BETTYPE_A(line))
    {
        ...
        {
            uint8 betnum[2] = {0};
        	bit2num(betpart->bitmap, 2, betnum, 0);
        	bit2num(betpart->bitmap + 2, 2, betnum + 1, 0);
            uint16 sortn = (betnum[0] < betnum[1])? betnum[0] * 10 + betnum[1] : betnum[1] * 10 + betnum[0];
        	bet = twoStarBCidx(sortn).numCount;
            for(int x = 0; x <= bet ; x++)
            {
                dindex[x] = twoStarBCidx(sortn).numArray[x];
            }
            break;
        }
        default:
...
uint16 rk_get4StarDirectSegIndex(BETLINE *line,uint16 dindex[])
{
    ...
    switch(GL_BETTYPE_A(line))
    {
        ...
        case BETTYPE_BC:
        {
        	...
        	bit2num(betpart->bitmap, 2, betnum, 0);
        	bit2num(betpart->bitmap + 2, 2, betnum + 1, 0);
        	bit2num(betpart->bitmap + 4, 2, betnum + 2, 0);
        	bit2num(betpart->bitmap + 6, 2, betnum + 3, 0);
            uint16 sortn = gl_sortArray(betnum,4);
        	bet = fourStarBCidx(sortn).numCount;
            for(int x = 0; x <= bet ; x++)
            {
                dindex[x] = fourStarBCidx(sortn).numArray[x];
            }
...
uint16  rk_get3StarDirectNumberIndex(BETLINE *line, uint16 dindex[])
{
    ...
    switch(GL_BETTYPE_A(line))
    {
        ...
        case BETTYPE_BC:
        {
        	...
        	bit2num(betpart->bitmap + 2, 2, betnum + 1, 0);
        	bit2num(betpart->bitmap + 4, 2, betnum + 2, 0);
            uint16 sortn = gl_sortArray(betnum,3);
        	bet = threeStarBCidx(sortn).numCount;
            for(int x = 0; x <= bet ; x++)
            {
                dindex[x] = threeStarBCidx(sortn).numArray[x];
            }
            break;
        }
        default:
...
bool gl_tty_rk_init(void)
{
	...
	for(num = 0; num < 10000; num++)
	{
		...
		sortn = gl_sortArray(array,4);

		fourStarBCidx(sortn).numArray[fourStarBCidx(sortn).numCount] = num;
		fourStarBCidx(sortn).numCount++;
	}
	...
	for(num = 0; num < 1000; num++)
	{
		...
		sortn = gl_sortArray(array,3);

		threeStarBCidx(sortn).numArray[threeStarBCidx(sortn).numCount] = num;
		threeStarBCidx(sortn).numCount++;
	}
	...
	for(num = 0; num < 100; num++)
	{
		...
		sortn = gl_sortArray(array,2);

		twoStarBCidx(sortn).numArray[twoStarBCidx(sortn).numCount] = num;
		twoStarBCidx(sortn).numCount++;
	}
