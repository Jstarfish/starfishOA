...
public class CenterSelectController {
    ...
    public String gameDraw(HttpServletRequest request, ModelMap model) {
        ...
        try {
            ...
            if (!StringUtils.isBlank(tsn)) {
                ...
                CenterSelectRes4001 resCheck = JSON.parseObject(resJson, CenterSelectRes4001.class);

                int statusCheck = resCheck.getRc();
                if (statusCheck == ResustConstant.OMS_RESULT_SUCCESS) {
                    /*
                * 根据语言将枚举转化需要用
    ...
    public String printInfo(HttpServletRequest request, ModelMap model, ExpirydateForm expirydateForm) {
        ...
        if (info != null) {
            ...
            String playSubType = null;
            if (info.getGameCode() == 14) {
                playSubType = info.getBetSection().toString().split("[|]")[1];
            }
                ...
                if (arr.length > 0) {
                    ...
                    if (pet.length > 0) {
                        for (int i = 0; i < pet.length; i++) {
                            ...
                            bet.setBettingnumber(req.changedBetString(playSubType, pet[i], req.getAlanguage()));
                            betList.add(bet);
                        }
                    }
