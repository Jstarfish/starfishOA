...
<body>
  <div id="title">FBS兑奖记录查询  </div>
    <div class="queryDiv">
       <form id="salegamepayinfoForm" action="expirydate.do?method=list" method="post">
            <div class="left">                      
                     <span>操作员用户：<input id="payer" name="payer" maxlength="20" class="text-normal"/></span>
                      <span>时间:开始<input id="startpayTime" name="startpayTime" value="${startpayTime }" class="Wdate" type="text" onFocus="WdatePicker({maxDate:'#F{$dp.$D(\'endpayTime\')}'})"/> 
    ...
    <div id="bodyDiv">
        <table class="datatable" id="exportPdfData">
			...
			<tr class="dataRow">
				...
				<td width="1%">&nbsp;</td>
				<td style="width:7%">
					<c:if test="${data.isSucess==0}">失败</c:if>
					<c:if test="${data.isSucess==1}">成功</c:if>
				</td>
				<td width="1%">&nbsp;</td>
