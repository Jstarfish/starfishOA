...
<body>
	...
	<div class="queryDiv">
		<form action="outlets.do?method=listOutlets" id="queryForm" method="POST">
			<div class="left">
				<span>Institution: <input name="institutionName" id="institutionName"
					class="text-normal" maxlength="12" value="${form.institutionName }"/></span> <span>Outlet Code: <input
					name="outletCode" id="outletCode" class="text-normal" maxlength="12" value="${form.outletCode }"/></span> <input
					type="button" value="Query" onclick="doSubmit();" class="button-normal"></input>
			</div>
			<div class="right">
				<table width="260" border="0" cellspacing="0" cellpadding="0">
					<tr style="height: 50px; line-height: 50px">
						<td align="right"><input type="button" value="New Outlet"
							onclick="showBox('outlets.do?method=createInit','New Outlet',650,800)"
			...
			<c:forEach var="items" items="${listOutlets }" varStatus="status">
				<tr class="dataRow">
					...
					<td style="width: *%"><c:if test="${items.status==1 }">
						...
						<span><a href="#"
								onclick="showBox('outlets.do?method=modifyInit&outletCode=${items.outletCode}','Modify Outlet',650,800)">Edit</a></span>&nbsp;|
							<span>Enable</span>&nbsp;|
