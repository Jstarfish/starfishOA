...
function doSubmit() {
	...
	var result = true;
 	...
 	if (!doCheck("operAmount", !isNull($("#operAmount").val()), 'Amount cannot be empty or 0')) {
		result = false;
	}else{ 
		if(!doCheck("operAmount", $("#operAmount").val() <= num5, 'Top-up amount cannot be larger than the sum of the credit limit and the account balance')){
		result = false;
	   }
	}
 	 if (!doCheck("password", !ifNull($("#password").val()), 'Password cannot be empty')) {
		result = false;
	}
...
<form action="outletInfo.do?method=OutletTopUps" id="OutletTopUpsForm" name="OutletTopUpsForm" method="POST">
	...
	<div class="pop-body">
		<table width="100%" border="0" cellspacing="0" cellpadding="0">
			...
			<tr>
				<td class="td" align="right">Top Up Amount：</td>
			...
			<tr>
				<td class="td" align="right">Balance after Top Up：</td>
	...
	<div class="pop-footer">
		<span class="left">
			<input id="okButton" type="button" value='Top Up' onclick="doSubmit();" class="button-normal"></input>
		</span> 
		<span class="right">			
