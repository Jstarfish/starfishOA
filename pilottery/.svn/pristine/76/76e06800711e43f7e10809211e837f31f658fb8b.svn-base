...
<c:forEach items="${user_privilege_list}" var="p">
	menu_add('${p.id}','${p.parentId}','${p.remark}','${p.url}','ifmain','');
</c:forEach>
function logout(){
    window.top.location.href = "${basePath}"+"?ver=*";
}

function logoutDialog(){
	showDialog('logout()',"退出系统","您确定要退出吗？");
}

function switchSystem(str) {
    switch(str) {
	    case "oms":
	    	window.location.href = "${omsUrl}";
	        break;
        case "pilottery":
        	window.location.href = "${pilotteryUrl}";
            break;
    }
		...
		<div class="header">
			...
			<div class="info">
				...
				<tr><td width="*%" class="left">
					欢迎, <span></span>
					用户:${sessionScope.current_user.realName}
					<span></span> 
					<span><c:forEach items="${sessionScope.user_role_list}" var="r">${r.name} &nbsp;&nbsp;</c:forEach></span>
				    <span class="exit"><a href="javascript:logoutDialog();">[退出]</a></span>
				</td>
				<td width="200px" class="right">
					<c:if test="${applicationScope.useCompany == 1}">
		        	</c:if>
		        	<c:if test="${applicationScope.useCompany == 2}">
		        		<img onclick="switchSystem('pilottery')" src="${basePath}/img/icon4.png" height="24px" width="24px" title="即开票平台"/>
			        	<img onclick="switchSystem('oms')" src="${basePath}/img/icon3.png" height="24px" width="24px" title="OMS管理平台"/>
		        	</c:if>
				</td>
				</tr>
                </table>
