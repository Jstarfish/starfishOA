...
public class CapitalRecordController {
	...
	public String listRecords(HttpServletRequest request, ModelMap model,CapitalRecordForm form){
		...
		Map<Integer,String> transFlowMap = capitalRecordService.getTransFlowList(currentUser.getInstitutionCode(),request);
		
		model.addAttribute("transFlowMap",transFlowMap);
		model.addAttribute("institutionList",institutionList);
		
		return LocaleUtil.getUserLocalePath("capital/institutionAccounts/listInstitutionTrans", request);
		
	}
	@RequestMapping(params = "method=queryListCapitalRecords")
