...
<body>
	...
	<div class="queryDiv">
		<form action="outlets.do?method=listOutlets" id="queryForm" method="POST">
			<div class="left">
				<span>机构: <select name="outletCode" id="outletCode"
					class="select-normal">
					<option value="">--所有--</option>
					<c:forEach items="${orgs}" var="obj">
						<option value="${obj.orgCode}" <c:if test="${obj.orgCode==form.outletCode}">selected="selected"</c:if>>${obj.orgName }</option>
					</c:forEach>                  
					</select>
				</span> 
				<span>状态:<select name="status" id="status" class="select-normal">
				<option value="0">--所有--</option>
				<option value="1" <c:if test="${form.status == 1}">selected="selected"</c:if>>启用</option>
			...
			<c:forEach var="items" items="${listOutlets }" varStatus="status">
				<tr class="dataRow">
					...
					<td style="width: *%"><c:if test="${items.status==1 }">
						...
						<span><a href="#"
								onclick="showBox('outlets.do?method=modifyInit&outletCode=${items.outletCode}','修改站点',550,800)">编辑</a></span>&nbsp;|
							<span>可用</span>&nbsp;|
