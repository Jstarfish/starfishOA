	...
	<form method="post" action="" id="checkPointParmat" name="checkPointParmat">
		...
		<div id="container">
			...
			<div class="header">
				<!--用户信息区开始-->
				<!--用户信息区结束-->
				<div id="title">Inventory Check</div>
			</div>
			<!--header end-->
			<!--开奖开始-->
			<div id="main">
													...
													<table width="90%" border="0" align="center" cellpadding="0" cellspacing="0" style="margin-top: 30px;">
														<tr>
															<td width="30%" style="border-bottom: 1px dashed #ccc; padding-bottom: 15px;" class="hoz">
																Differences :<span class="lz2">${result.cbeforeNum-result.cafterNum} </span>tickets
															</td>
															<td width="30%" style="border-bottom: 1px dashed #ccc; padding-bottom: 15px;">
																Before Check : <span class="lz2"> ${result.cbeforeNum}</span>tickets
															</td>
															<td>
																After Check : <span class="lz2"> ${result.cafterNum}</span>tickets
															</td>
														</tr>
														<tr>
															<td>
																Profit and loss : <span class="lz2">${result.cbeforeNum-result.cafterNum}</span>tickets									
															</td>
															<td colspan="2">
																Resulst : 
																<span class="lz2">
																	<c:choose>
																		...
																		<c:when test="${result.cresult==2}">Deficits</c:when>
												...
												<td>
													<div style="margin: 10px 40px; float: right;">
														<c:if test="${result.cresult==2}">
															<button id="dregeist" type="button"  class="gl-btn2" disabled="disabled">Damage Register</button>
														</c:if>
