...
public class LocaleUtil {
			...
			if (user != null) {
				...
				if (lg == UserLanguage.ZH) {
					obj = ReflectUtil.getStaticProperty(
							"cls.taishan.common.constants.EnumConfigZH", enumName);
					
				} else if (lg == UserLanguage.EN) {
					obj = ReflectUtil.getStaticProperty(
							"cls.taishan.common.constants.EnumConfig", enumName);
				}
	 ...
	 * 获取当前用户所选择的语言
	 */
	public static UserLanguage getUserLang(HttpServletRequest request) {
		User user = (User) request.getSession().getAttribute(SysConstants.CURR_LOGIN_USER_SESSION);
		return user.getUserLang();
	}
	
	public static boolean isChinese(HttpServletRequest request) {
		User user = (User) request.getSession().getAttribute(SysConstants.CURR_LOGIN_USER_SESSION);
		return user.getUserLang() == UserLanguage.ZH;
	}
}
