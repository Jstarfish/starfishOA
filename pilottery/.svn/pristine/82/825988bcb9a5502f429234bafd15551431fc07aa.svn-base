 ...
 * 参数：
 * 返回值：
 * 功能描述：对指定文件生成MD5文件,并返回md5值
 ******************************************************************************************/
int md5_file(const char *file, char *md5)
{
	int ret = 0;

	char fileMd5[PATH_MAX] = {0};
    strcpy(fileMd5, file);
    strcat(fileMd5, ".md5");
    ...
    ret = stat(file, &stat_t);
    if (ret != 0)
    {
        perrork("stat(%s) failed. Reason:%s.", file, strerror(errno));
        return -1;
    }

    if((fd = open(file, O_RDONLY)) == -1)
    {
        perrork("open(%s) failed. Reason:%s.", file, strerror(errno));
        return -1;
    }
    ...
    int count = 0;
    char buf[1024*32];
    while ((count = safe_read(fd, buf, sizeof(buf))) > 0)
    {
    ...
    if (count < 0)
    {
        perrork("safe_read(%s) failed. Reason:%s.", file, strerror(errno));
        return -1;
    }

    if ((fdMd5 = open(fileMd5, O_WRONLY|O_CREAT|O_TRUNC, S_IRUSR|S_IWUSR|S_IRGRP|S_IROTH)) == -1)
    {
         perrork("open(%s) failed. Reason:%s.", fileMd5, strerror(errno));
         return -1;
    }
    ...
    if (full_write(fdMd5, digest_str, sizeof(digest_str) - 1) != sizeof(digest_str) - 1)
    {
        perrork("full_write(%s) failed. Reason:%s.", fileMd5, strerror(errno));
        close(fdMd5);
        return -1;
    }
    close(fdMd5);
...
int get_md5_file(const char *file, char *md5)
{
    ...
    int fd;
    if((fd = open(fileMd5, O_RDONLY)) == -1)
    {
        perrork("open(%s) failed. Reason:%s.", fileMd5, strerror(errno));
        return -1;
    }

    char digest_str[32+1] = {0};
    int len = safe_read(fd, digest_str, 32);
    if (sizeof(digest_str) - 1 != len)
    {
        perrork("safe_read(%s) failed.len[%d] Reason:%s.", fileMd5, len, strerror(errno));
        close(fd);
        return -1;
    }
    close(fd);
 ...
 * 参数：
 * 返回值：
 * 功能描述：对指定文件计算MD5值, 并与此文件的MD5文件的值进行比较
 ******************************************************************************************/
int md5_file_verify(const char *file)
{
    char md5_a[32];
    char md5_b[32];
    if (0 > md5_file_calc(file, md5_a)) {
        log_error("md5_file_calc() error. %s", file);
        return -1;
    }
    if (0 > get_md5_file(file, md5_b)) {
        log_error("get_md5_file() error. %s", file);
        return -1;
    }
    if (0 != strncmp(md5_a,md5_b,32)) {
        log_error("md5 check failure. %s", file);
        return -1;
    }
    return 0;
}
               ...
               unsigned char *response,  size_t resp_len)
{
    ...
    {
        MD5Init (&ctx);
        MD5Update (&ctx, (unsigned char *)password, pass_len);
