   ...
   case p_paid_type
      when 3 then                                                       -- 销售站兑奖
         ...
         if (v_org_type <> '00' and v_org_type = eorg_type.company and f_get_sys_param(16) = '1') or (v_org_type = eorg_type.agent) then
            ...
            v_comm_amount := v_reward_amount * v_comm_rate;
            ...
            -- 组织机构增加流水
            -- 奖金
            p_org_fund_change(v_org, eflow_type.paid, v_reward_amount, 0, v_pay_flow, v_balance, v_balance);

            -- 佣金
            if v_comm_amount > 0 then
               p_org_fund_change(v_org, eflow_type.org_comm_pay, v_comm_amount, 0, v_pay_flow, v_balance, v_balance);
            end if;
         end if;
