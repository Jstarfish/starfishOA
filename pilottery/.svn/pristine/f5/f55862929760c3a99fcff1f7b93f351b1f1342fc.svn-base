#ifndef GL_KOCTTY_MATCH_H__
#define GL_KOCTTY_MATCH_H__
...
int gl_koctty_betlineMatch(
		const BETLINE *betline,
		DIVISION_PARAM *division,
		uint32 matchRet[]);

int gl_koctty_betpartMatch(
		const BETLINE *betline,
		const DIVISION_PARAM *divparam);

int gl_koctty_DSPartMatch(
		const BETLINE *betline,
		const DIVISION_PARAM *divparam);

int gl_koctty_FSPartMatch(
		const BETLINE *betline,
		const DIVISION_PARAM *divparam);

int gl_koctty_YXFSPartMatch(
		const BETLINE *betline,
		const DIVISION_PARAM *divparam);

int gl_koctty_HZPartMatch(
		const BETLINE *betline,
		const DIVISION_PARAM *divparam);
