...
function deleteIssue(gameCode,issueNumber,isPublish){ 
	$("#gameManagementForm").attr("action","issueManagement.do?method=deleteIssue&fl=confirm&gameCode="+gameCode+"&issueNumber="+issueNumber+"&isPublish="+isPublish);
        ...
        <tr class="dataRow">
        	...
        	<td width="330">
                ...
                <c:when test="${row.isPublish==0 && row.issueStatus==0 && (row.gameCode==6 || row.gameCode==7)}">
               		<a href="#" onclick="showBox('issueManagement.do?method=editCalcWinningParam&gameCode=${row.gameCode}&issueNumber=${row.issueNumber}','算奖参数',240,450)">算奖参数</a> |
                </c:when>
                <c:when test="${row.issueStatus==13}">
                </c:when>
                <c:otherwise>
