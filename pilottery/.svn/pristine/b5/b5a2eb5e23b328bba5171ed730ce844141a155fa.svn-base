...
var hint1 = '* 非空，数字，长度不超过10!';//非空，数字，长度不超过10!
var hint2 = '* 此编码不存在或无效，请输入有效编码!';//此编码不存在，请输入有效编码!
var hint3 = '* 非空，数字，长度不超过2!';//非空，数字，长度不超过2!
var hint4 = '* 非空，数字或英文字符，长度不超过20!';//非空，数字或英文字符，长度不超过20!
var hint5 = '* 请按AA:BB:CC:DD:EE:FF格式输入!';//请按AA:BB:CC:DD:EE:FF格式输入!
var hint6 = '* 请选择！';//请选择！
var hint7 = '* MAC地址冲突，请重新输入！';//MAC地址冲突，请重新输入！
var select = '* 请选择';
function doSubmit(){
	var result = true;
	...
	if (!doCheck("trainingmode",/^[^2]$/,hint6)) {
		result = false;
	}
		...
		<div class="pop-body">
	        <table width="100%" border="0" cellspacing="0" cellpadding="0" >
	            <tr>
					...
					<c:if test="${agencyCode==null}">
						<td align="right" width="25%">归属销售站编码：</td>
				...
				<tr>
					<td align="right">终端机标识码：</td>
					<td align="center"><input name="uniqueCode" class="text-big" id="uniquecode" type="text"/></td>
					<td><span id="uniquecodeTip" class="tip_init">* 非空，数字或英文字符，长度不超过20!</span></td>
				</tr>
				<tr>
					<td align="right">终端机MAC地址：</td>
				...
				<tr>
					...
					<td align="center">
						<select name="terminalType" class="select-big" id="terminalType">
							<option value="-1" selected="selected">选择终端型号</option>
				...
				<tr>
					<td align="right">是否培训模式：</td>
					<td align="center">
						<select name="trainingMode.value" class="select-big" id="trainingmode" >
							<c:forEach items="${trainingmode}" var="m">
								<option value="${m.key}">${m.value }</option>
							</c:forEach>
					     </select>
					</td>
					<td><span id="trainingmodeTip" class="tip_init"></span></td>
				</tr>
				<tr>
