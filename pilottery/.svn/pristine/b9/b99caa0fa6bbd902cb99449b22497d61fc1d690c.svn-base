        ...
        const SUBTYPE_PARAM* subparam)
{
    ...
    switch (bettype)
    {
        ...
        case BETTYPE_BC:
        {
        	ret = gl_koctty_BCPartVerify(bp,subparam);
        	break;
        }
		case BETTYPE_BD:
        {
        	ret = gl_koctty_BDPartVerify(bp,subparam);
        	break;
        }
        ...
        const SUBTYPE_PARAM *subparam)
{
    ...
    {
        default:
            log_warn("gl_koctty_BCPartVerify mode[%d] not support!",bp->mode);
            ret = -1;
            break;
        case MODE_JC:
            //范围
            maxnum = bitHL2num(bp->bitmap, 0, bp->size, 1, 0);
            if ((maxnum > rangeMax) || (maxnum < rangeMin)) {
                log_warn("gl_koctty_BCPartVerify JC range fail! maxnum[%d]",maxnum);
                ret = -1;
                break;
            }
            minnum = bitHL2num(bp->bitmap, 0, bp->size, 0, 0);
            if ((minnum > rangeMax) || (minnum < rangeMin)) {
                log_warn("gl_koctty_BCPartVerify JC rangeL fail! minnum[%d]",minnum);
                ret = -1;
                break;
            }
            //数量
            bitcnt = bitCount(bp->bitmap, 0, bp->size);
            if ((bitcnt > countMax) || (bitcnt < rangeCnt)) {
                log_warn("gl_koctty_BCPartVerify JC rangeL fail! bitcnt[%d]",bitcnt);
                ret = -1;
                break;
            }

            log_debug("gl_koctty_BCPartVerify JC OK");
            break;
    }
	...
	const SUBTYPE_PARAM *subparam)
{
	...
	{
	default:
		log_warn("gl_koctty_BDPartVerify mode[%d] not support!", bp->mode);
		ret = -1;
		break;
	case MODE_FD:
		...
		bitcnt =  bp->size / 2;
		if ((bitcnt > countMax) || (bitcnt < rangeCnt)) {
			log_warn("gl_koctty_BDPartVerify rangeL fail! bitcnt[%d]", bitcnt);
			ret = -1;
			break;
		}
		uint8 outnum[10] = { 0 };
		...
		for (i = 0; i < bitcnt; i++)
		{
			bit2num(bp->bitmap + 2 * i, 2, outnum + i, 0);
		}
		for (i = 0; i < bitcnt; i++)
		{
			if (outnum[i] > rangeMax)
			{
				log_warn("gl_koctty_BDPartVerify outnum[%d]", outnum[i]);
				ret = -1;
			}
		}

		log_debug("gl_koctty_BDPartVerify OK");
		break;
	}
