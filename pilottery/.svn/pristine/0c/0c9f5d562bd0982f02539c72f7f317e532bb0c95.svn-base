...
function initTerBatch(planCode,batchNo,planName) {
	showBox('termination.do?method=detailsBatchPrint&planCode='+planCode+'&batchNo='+batchNo+"&planName="+planName,'Batch Details',365,650);
}
/* function initTerBatch(planCode,batchNo,planName) {
	$
			.post(
...
<body>
	<!-- 顶部标题块 -->
	<div id="title">List of Batch</div>
	<!-- 列表表头块 -->
	<div id="headDiv">
				...
				<td>
					<table class="datatable">
						<tr class="headRow">
							...
							<td style="width: 18%">Plan Code</td>
							<th width="1%">|</th>
							<td style="width: 18%">Plan Name</td>
							<th width="1%">|</th>
							<td style="width: 18%">Batch</td>
							<th width="1%">|</th>
		...
		<table class="datatable">
			<c:forEach var="data" items="${batchList}" varStatus="status">
				<tr class="dataRow">
					...
					<span><a href="#" onclick="initTerBatch('${data.planCode}','${data.batchNo}','${data.planName}');">Termination</a></span>&nbsp;|
					<span>
					<a href="#"	onclick="showBox('termination.do?method=detailsBatch&planCode=${data.planCode}&batchNo=${data.batchNo}','Batch Details',380,650)">Detail</a></span>&nbsp;|
					<span style="color:gray">Print</span>
					</c:if>
					<c:if test="${data.status==3 }">
					<span style="color:gray">Termination</span>&nbsp;|
					<span>
					<a href="#"	onclick="showBox('termination.do?method=detailsBatch&planCode=${data.planCode}&batchNo=${data.batchNo}','Batch Details',366,680)">Detail</a></span>&nbsp;|
					<span><a href="#" onclick="showPage('termination.do?method=print&planCode=${data.planCode}&batchNo=${data.batchNo}&planName=${data.planName}','Print')">Print</a></span>
					</c:if>
				</tr>
			</c:forEach>
		</table>
