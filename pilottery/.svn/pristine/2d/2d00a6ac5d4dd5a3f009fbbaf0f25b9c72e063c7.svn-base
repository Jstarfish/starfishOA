...
//		var cNameObj = document.getElementById(nameforresvobj);//存地域名的标签id
		if (isNullObj(obj))
			...
			showMsg('信息提示',idforresvobj + '对象异常',"warn");
		obj.value = "";
		...
		if (isNullObj(obj)) {
			...
			showMsg('信息提示',nameforresvobj + '对象异常',"warn");
		...
		if (isNullObj(obj))
			//alert(levelId + '对象异常');
			showMsg('信息提示',levelId + '对象异常',"warn");
		...
		if (isNullObj(obj))
			...
			showMsg('信息提示',idforlist + '对象异常',"warn");
	...
	function addTreeOjbs() {
		...
		if (!isNullObj(zTree)) {
			...
			showMsg('信息提示','invalid tree',"warn");
...
//		}
	...
	function sendIMInfo(){
		...
		if (ccontent.value.length == 0) {
			//alert("请输入消息内容！");
			showMsg('信息提示','请输入消息内容，长度限200字符',"warn");
			return;
		}
