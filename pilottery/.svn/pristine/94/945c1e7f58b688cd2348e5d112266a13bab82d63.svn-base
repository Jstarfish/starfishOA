...
public class PlanController {
	...
	public String deleteOutlets(HttpServletRequest request) {
		...
		if (haveBatch != 0) {
			if(lg == UserLanguage.ZH){
				map.put("reservedSuccessMsg", "该方案有批次,不允许被删除!");
			}else{
	...
	public String importBatch(HttpServletRequest request , ModelMap model) throws Exception {
		...
		try {
			...
			if (batchPub.getErrorCode() == 0) {
				...
				String msg = LocaleUtil.getUserLocaleErrorMsg(batchPub.getErrorMessage(),request);
				model.addAttribute("system_message", msg);
				return LocaleUtil.getUserLocalePath("common/errorTip", request);
			}
		} catch (Exception e) {
